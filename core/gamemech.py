...
logging.basicConfig(filename=f"{data_ex}/log.log", level=logging.DEBUG, format="[%(filename)s:%(lineno)s - %(funcName)20s() ] %(message)s")
...
class GameMech(Widget):
	...
	def __init__(self,**kwargs):
		...
		self.network = {}
		self.cdsh = {}
		self.sd = {"field_btn_fill": False, "alabel": "","lremove":{"p":[],"s":[],"t":[],"l":[],"n":[],"tr":[],"c":[]}}
		self.gd = {}
		self.cd = {}
		self.cpop = {}
		self.dpop = {}
	...
	def cancel_version_check(self, *args):
		...
		if not self.gd["payed"] and "confirmbefore" not in self.gd["effect"]:
			...
			if "ClockL" in self.gd["pay"]:
				...
				if card.ind[-1] != self.gd["active"]:
					self.gd["drev"] = True
				if "Accelerate" in self.gd["pay"]:
					self.gd["pay"].insert(self.gd["pay"].index("Accelerate") + 1, 0)
	...
	def pay_condition_done(self, dt=0):
		...
		ind = self.gd["ability_trigger"].split("_")[1]
		if "Accelerate" in self.gd["pay"]:
			_ = self.gd["pay"].index("Accelerate")
			if isinstance(self.gd["pay"][_ + 1], int):
				if self.gd["pay"][_ + 1]:
					self.check_auto_ability(accel=ind, stacks=False)
		...
		if "ACT" in self.gd["ability_trigger"]:
			...
			Clock.schedule_once(self.ability_effect,dt)
	...
	def check_auto_ability(self, dt=.0, rev=[""], atk="", play="", wait="", revive="", trigger="", cnc=("", False), refr="", lvup="", stacks=True, act="", dis="", sav="", dmg=0, rst=[], brt=("", 0), lvc="", chg="",accel=""):
		std = ("", "", "")
			...
			if "Stage" in auto[1] and "Open" in auto[1]:
				...
				if "Opp" in auto[1]:
					if p == "1":
						...
						o = "1"
				if "Center0" in auto[1]:
					stage1 = []
					if self.pd[o]["Center"] =="":
						stage1.append("")
				elif "Center" in auto[1]:
	...
	def import_mat(self, player, mat="mat"):
		...
		try:
			...
			if self.gd["DLimg"]:
				...
				self.mat[player]["id"] = "nodl"
			self.decks[player][1] = self.mat[player]["id"]
			self.mat[player]["mat"].import_mat(sp[self.mat[player]["id"]], self.mat[player]["per"])
		self.scale_mat(t=False, player=player)
	...
	def popup_deck(self, t="start", dt=.0, *args):
		...
		if t == "start":
			...
			for mat in sorted(se["main"]["m"]):
				...
				source = "main"
				img = sp[mat]["img"]
				if "mat_mat" in img or "mat_nodl" in img or "mat_matj" in img:
					source = "other"
					if "." in img:
						img = img[:-4]
			...
			if "d" in t:
				...
				for ind in decks:
					if "sspace" in ind:
						...
						self.dpop[f"id{ind}"].update_image(self.gd["DLimg"])
				...
				if not self.gd["DLimg"] and "mat" in decks:
					decks.remove("mat")
				if not self.gd["DLimg"] and "matj" in decks:
					decks.remove("matj")
				if not self.gd["DLimg"]:
				...
				for ind in decks:
					if "sspace" in ind:
						...
						self.dpop[f"im{ind}"].update_image(self.gd["DLimg"])
				...
				if item.startswith("m_"):
					...
					if "text" in item:
						...
						if "d" in item and "ran" not in item:
							...
							self.network[item].update_image(self.gd["DLimg"])
					elif "m" in item:
	...
	def check_internet(self, url="", timeout=2, ver=False):
		...
		if ver and _:
			Clock.schedule_once(self.check_version_start)
		elif ver and not _:
			self.main_scrn.disabled = False
			self.sd["text"]["popup"].dismiss()
		elif not ver and not _:
			Clock.schedule_once(partial(self.popup_text, "no_internet"))
	...
	def mconnect(self, btn, dt=0):
		...
		if "version" in var or self.check_internet():  
			...
			if "connect" in var:
				...
				if self.decks["1"][2]:
					_ = choice([s for s in sd.keys() if sd[s]["c"]])
				...
				if _.startswith("CEJ"):
					s1 = "'"
					s2 = '"'
					deck1 = f"{_}~{str(sd[_]['deck'])[1:-1].replace(s1, '').replace(s2, '').replace(' ', '')}"
				...
				if sd[_]['l'] == "":
					dset = f"0b{sd[_]['n'][:2]}{sd[_]['t']}"
				else:
					dset = f"0{sd[_]['l']}{sd[_]['n'][:2]}{sd[_]['t']}"
				...
				if (self.gd["active"] == "2" and not self.gd["rev"]) or (self.gd["rev"] and self.gd["active"] == "1"):
					...
					if self.net["var1"] == "atk decla":
						_ = list_str(self.net['var'])
					else:
				...
				dat = {"0": "q"}
				Clock.schedule_once(self.cancel_version_check, 5)
			elif "down" in var:
				if self.net["var"][1] <= 0:
					Clock.schedule_once(partial(self.popup_text, "Loading"), ability_dt)
		...
		for deck1 in sd:
			...
			if "main" in source:
				if exists(f"{cache}/{img}"):
					...
					self.dpop[f"id{deck1}"].img_btn = f"{cache}/{img}"
				else:
					self.dpop[f"id{deck1}"].source = f"atlas://{img_in}/other/grey"
					self.dpop[f"id{deck1}"].img_btn = f"atlas://{img_in}/other/grey"
			else:
				self.dpop[f"id{deck1}"].source = f"atlas://{img_in}/{source}/{img}"
				self.dpop[f"id{deck1}"].img_btn = f"atlas://{img_in}/{source}/{img}"
	...
	def clear_loaded_game(self, dt=0):
		...
		if "Load" in self.sd["text"]["c"]:
			self.main_scrn.disabled = False



	def check_version_start(self,dt=0):
	...
	def check_pos(self, ind, wig=True):
		...
		elif ind in self.pd[ind[-1]]["Stock"]:
			...
			if "Marker" in self.cd[ind].pos_new:
				...
				if self.gd["attacking"][0]:
					if ind in self.gd["attacking"][0]:
						self.gd["attacking"][0] = ""
					elif self.gd["attacking"][4] == self.cd[ind].pos_new[0] and self.gd["attacking"][3] == self.cd[ind].pos_new[-1]:
						self.gd["attacking"][4] = ""
	...
	def salvage(self, dt=0):
		...
		if self.gd["p_c"] != "" and (not self.gd["target"] or self.gd["p_again"]):
			...
			for r in range(self.gd["salvage"]):
				...
				player = ind[-1]
				if "Waiting" not in self.gd["effect"] and (player == "2" or (player == "1" and imd[-1] == "2")):
					idm.append(ind)
	...
	def popup_text(self, c="", dt=0):
		...
		if "Debugtext" in c:
			...
			if "Select" in status:
				if len(self.gd["select_btns"]) > 0:
					...
					elif "Center0" in status:
						n += " in the left position of your center stage"
						e += ""
		...
		if "Debugtext" in c or "version" in c or ("choice" in self.gd["effect"] and c == "Move"):
		...
		if self.multi_info["t"]:
			...
			if self.decks["img_pop"]:
				...
				if f"id{self.decks['dbuilding']}" in self.dpop:
					...
					self.dpop[f"id{self.decks['dbuilding']}"].img_btn = str(self.decks["st"]["image_btn"].source)
	...
	def get_fields(self, status=""):
		...
		if "Osite" in status:
			...
			fields = [self.pd[player]["Center"][0]]
		elif "Center" in status:
	...
	def select_field(self, p=False, *args):
		...
		if "Clock" in status:
			...
			if "Change" in status:
				...
				fields = [self.gd["stage"][1]]
			elif "Center0" in status:
				fields = [self.gd["stage"][0]]
			elif "Center" in status:
			...
			if "Open" in status:
				fields = [fd for fd in fields if fd != "" and self.pd[player][fd[:-1]][int(fd[-1])] == ""]
			if "separate" in self.gd["effect"]:
	...
	def janken_start(self, *args):
		...
		for i in reversed(range(2)):
			for _ in self.gd["janken_choice"]:
				self.sd["janken"][f"j{_}{i}"].update_image(self.gd["DLimg"])
	...
	def ability_effect(self, *args):
		...
		if self.gd["done"] and self.gd["do"][0] > 0:
			...
			if self.gd["effect"][0] == -16:
				...
				if "xopp" in self.gd["effect"]:
					self.gd["effect"][0] = len(self.cont_cards(["opp"], ind))
			...
			elif not self.gd["p_c"] and self.gd["com"] and ((ind[-1] == "2" and "oppturn" not in self.gd["effect"]) or ("oppturn" in self.gd["effect"] and ind[-1] == "1")):
				self.gd["p_c"] = "Discard"
			...
			if self.gd["effect"][0] == -16:
				...
				if self.net["game"] and "plchoose" in self.gd["effect"]:
					if ind[-1] == "1":
						...
						self.net["send"] = False
						self.net["var"] = list(self.net["act"])
						self.net["var1"] = "plchoose salvage"
						if not self.poptext:
							Clock.schedule_once(partial(self.popup_text, "waitingser"))
						self.mconnect("act")
						return False
			...
			if self.gd["effect"][0] > 0 and not self.gd["target"] and ind[-1] == "1":
				...
				if "AI_Stage" in pick:
					...
					for x in pick[inx + 1]:
						self.gd["target"].append(x)
				for x in range(self.gd["effect"][0]-len(self.gd["target"])):
					self.gd["target"].append("")
	...
	def check_cont_ability(self, dt=0, act=True, *args):
		...
		for player in list(self.pd.keys()):
			...
			for ind in stage + [player]:
				...
				card = self.cd[ind]
				for _ in self.sd["lremove"]:
					self.sd["lremove"][_] = []

				for _ in card.power_c:
					if _[1] < 0:
						if self.cont_remove(_, ind, player) and _ not in self.sd["lremove"]["p"]:
							self.sd["lremove"]["p"].append(_)

				for _ in card.soul_c:
					if _[1] < 0:
						if self.cont_remove(_, ind, player) and _ not in self.sd["lremove"]["s"]:
							self.sd["lremove"]["s"].append(_)

				for _ in card.text_c:
					if len(_) > 2 and -9 < _[1] < 0:
						if self.cont_remove(_, ind, player) and _ not in self.sd["lremove"]["t"]:
							self.sd["lremove"]["t"].append(_)

				for _ in card.level_c:
					if _[1] < 0:
						if self.cont_remove(_, ind, player) and _ not in self.sd["lremove"]["l"]:
							self.sd["lremove"]["l"].append(_)

				for _ in card.trait_c:
					if _[1] < 0 and _[1] != -66:
						if self.cont_remove(_, ind, player) and _ not in self.sd["lremove"]["tr"]:
							self.sd["lremove"]["tr"].append(_)

				for _ in card.name_c:
					if _[1] < 0:
						if self.cont_remove(_, ind, player) and _ not in self.sd["lremove"]["n"]:
							self.sd["lremove"]["n"].append(_)

				for _ in card.cost_c:
					if _[1] < 0:
						if self.cont_remove(_, ind, player) and _ not in self.sd["lremove"]["c"]:
							self.sd["lremove"]["c"].append(_)

				for _ in self.sd["lremove"]["p"]:
					card.power_c.remove(_)
				for _ in self.sd["lremove"]["s"]:
					card.soul_c.remove(_)
				for _ in self.sd["lremove"]["t"]:
					card.text_c.remove(_)
				for _ in self.sd["lremove"]["l"]:
					card.level_c.remove(_)
				for _ in self.sd["lremove"]["tr"]:
					card.trait_c.remove(_)
				for _ in self.sd["lremove"]["n"]:
					card.name_c.remove(_)
				for _ in self.sd["lremove"]["c"]:
					card.cost_c.remove(_)
				...
				for ind in self.pd[player][field]:
					...
					for item in card.text_c:
						if item[0].startswith(cont_ability) and item[1] > -9 and item[1] != 0:
							...
							if len(effect) < 4:
								continue
							if field == "Clock" and "Alarm" not in effect:
								continue
		...
		for _ in reversed(self.power_zero):
			self.gd["no_cont_check"] = True
			self.send_to_waiting(_)
		if "1" in self.gd["active"]:
	...
	def check_cont_hand(self, player):
		for ind in self.pd[player]["Hand"]:
			...
			for _ in self.sd["lremove"]["l"]:
				card.level_c.remove(_)
			if self.sd["lremove"]["l"]:
				card.update_level()
			for _ in self.sd["lremove"]["c"]:
				card.cost_c.remove(_)
			if self.sd["lremove"]["c"]:
				card.update_cost()
	...
	def give(self, dt=0):
		...
		elif self.gd["effect"][0] == -1 or self.gd["effect"][0] == -2:
			if idm[-1] == "1" or (idm[-1] == "2" and self.gd["com"]):
				if "Battle" in self.gd["effect"] and self.gd["effect"][0] == -1 and self.gd["attacking"][1] == "f":
					...
					for ta in self.cont_times(self.gd["effect"], self.cont_cards(self.gd["effect"], idm), self.cd):
						self.gd["target"].append(ta)
			if self.gd["effect"][0] == -2:
				if idm in self.gd["target"]:
					self.gd["target"].remove(idm)
			self.gd["effect"][0] = len(self.gd["target"])
		if "this" in self.gd["effect"] and self.gd["effect"][0] > 0:
			if idm[-1] == "1" or (idm[-1] == "2" and self.gd["com"]):
				self.gd["target"].append(idm)
	...
	def damage_step(self, *args):
		...
		self.gd["dmg"] = 0
		self.gd["drev"] = True
		if self.gd["attacking"][0] and self.gd["attack_t"][self.gd["attacking"][1]][self.gd["attacking"][2]] and card.soul_t > 0:
			d = True
			for text in card.text_c:
				if text[0].startswith(cont_ability) and text[1] != 0 and text[1] > -9:
					eff = ab.cont(text[0])
	...
	def hand_size(self, owner, move=True):
		...
		if self.mat[owner]["mat"].actual[1] > self.mat[owner]["mat"].size[1]:
			height += self.mat[owner]["mat"].actual[1] - self.mat[owner]["mat"].size[1]
