...
class Labelbtn(RelativeLayout):
	def __init__(self, size=(0, 0), **kwargs):
		...
		for n in range(2):
			self.ian[str(n)] = Image(source=f"atlas://{img_in}/other/blank", allow_stretch=True, size_hint=(None, None),
			                         size=(
			                         self.test["1"].texture.size[0] * 1.05, self.test["1"].texture.size[1] * 1.05))

		self.btn = Button(size=self.size, pos=self.pos, size_hint=(None, None), halign='left', valign='top',
		                  text_size=(self.size[0] * 0.95, self.size[1] * 0.95), markup=True, height=self.size[1])
	...
	def replaceImage(self):
		...
		for n in range(len(self.ian), len(self.btn.anchors)):
			self.ian[str(n)] = Image(source=f"atlas://{img_in}/other/blank", allow_stretch=True,
			                         size_hint=(None, None),
			                         size=(
			                         self.test["1"].texture.size[0] * 1.05, self.test["1"].texture.size[1] * 1.05))
		...
		for item in self.btn.anchors:
			...
			self.ian[str(self.inx)].pos = (
				self.size[1] / 9 + self.btn.anchors[item][0] + self.btn.x,
				self.btn.size[1] - self.size[1] / 15 - self.btn.anchors[item][1] - self.test[item[-1]].texture.size[
					1] + self.btn.y)
			self.inx += 1
