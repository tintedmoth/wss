...
level_pos = (0.015, 0.015)  
power_bar = (0.015, 0.015)  
power_text = (0.12, 0.010)  
icon_pos = (0.035, 0.010)  
soul_pos = (0.015, 0.010)  
trigger_pos = (0.015, 0.015)  
colour_pos = (0.015, 0.015)  
ability_pos = (0.015, 0.015)  
marker_pos = (0.015, 0.015)  
...
class Card(RelativeLayout):
	def __init__(self, code="", card=(100, 100), owner="", per=1, data="", **kwargs):
		...
		with self.canvas:
			...
			self.soul_l = CoreLabel(text="", text_size=(soul_size[0] * 2.5, self.power_b.size[1]), color=(1, 1, 1, 1), outline_width=1.9, halign='left', valign='middle', font_size=self.power_b.size[1] * 0.8)
			self.soul_l.refresh()
			self.soul_r = Rectangle(texture=self.soul_l.texture, size=self.soul_l.texture.size, pos=(self.power_b.pos[0] + soul_size[0]*1.1, self.power_b.pos[1] + self.power_b.size[1]*.95))
	...
	def update_level(self):
		if self.card != "Climax":
			...
			with self.canvas:
				...
				self.level_r.pos = (self.level_i.pos[0] + (self.level_i.size[0] - self.level_r.size[0]) / 2, self.level_i.pos[1] + self.level_i.size[1] * 0.12)
		else:
...
class CardImg(RelativeLayout):
	...
	def update_ability(self):
		...
		with self.canvas:
			...
			for text in self.text_c:
				if text[1] != 0 and text[1] != -3 and text[1] != -2 and len(ability) < self.max_ability:  
					if text[0].startswith(cont_ability):
						ability.append(self.img_cont)
					elif text[0].startswith(auto_ability):
						ability.append(self.img_auto)
