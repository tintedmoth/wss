...
class Ability:
	...
	def pay(self, a=""):
		...
		if "choose 1 \"" in t and "\" in your waiting room and put it face-down as marker under a \"" in t and "\" that had no markers]" in t:
			...
			if "if this is in memory" in a.lower():
				...
				o = ["Flip", 0, "down"]
		elif ("reveal a \"" in t or "reveal 1 character with \"" in t) and ("\" in your hand and put it in your stock]" in t or "\" from your hand and put it face-down under this as marker]" in t or "\" from your hand]" in t or "\" in your hand]" in t or "\" from your hand to your opponent]" in t or "\" from hand]" in t or "\" in name in your hand]" in t):
	...
	def req(self, a="", x=0, m=0, ss=(), nn=0, h=(), my=(), wr=(), cx=(), bb=()):
		...
		if "choose 1 \"" in t and "\" in your waiting room and put it face-down as marker under a \"" in t and "\" that had no markers]" in t:
			...
			if len(my) > 1 and any(h[3] and h[4] for hn in my):
				ro = True
		elif ("reveal a \"" in t or "reveal 1 character with \"" in t) and ("\" from your hand]" in t or "\" in your hand]" in t or "\" in your hand and put it in your stock]" in t or "\" from your hand to your opponent]" in t or "\" from hand]" in t or "\" from your hand and put it face-down under this as marker]" in t or "\" in name in your hand]" in t):
			if len(h) >= 1:
	...
	def event(self, a=""):
		...
		if self.ablt:
			...
			return []
	def auto(self, a="", p="", r=("0", "0", ""), v=("", ""), passed=False, lvop=(0, 0), cx=("", "0", ""), text=([], []), pos=("", "", "", ""), n="",begin=(), tr=([], []), z=(0, 0, ""), act="", cnc=("0", False), pp=0, dis=("", ""), nr=("", ""), atk="", dmg=0, nmop=("", ""), rst=[], lvc=("", ""), refr="", sav=("", ""), brt=("", 0), lr=(0, 0), lvup="", baind=("0", "0"), csop=(0, 0), suop=("", ""), trop=([], []), ty=("", [], "", ""), std=("", "", ""), opp=""):
		""":param a: ability (string)
		...
		if dis[0]:
			...
			if "when your opponent levels up" in t or "when your opponent levels-up" in t:
				if lvup not in r[0][-1]:
					...
					if "\" is in your climax area" in aa:
						self.ablt = 0
						if f"\"{cx[0].lower()}\" is in your climax area" in aa:
							self.ablt = 9
			...
			if "during attack phase" in t and p in self.attack:
				if "when your or your opponent's character is returned from the waiting room to hand" in t:
					if "all of that player's characters get - soul" in t:
						return [-19, self.digit(a), x, "soul", "player", sav[0][-1]]
		elif brt[0]:
			...
			if r[0] != r[1] and r[0][-1] == r[1][-1] and self.name(a, self.cond[1], s='n') in nr[1] and "Hand" in pos[2] and any(field in pos[3] for field in self.stage) and "Stand" in v[1]:
				...
				self.cond[1]+=2
				self.target = r[1]
		elif "when your other character with  or  is placed from hand to the stage" in t and "» or «" in aa:
			...
			if r[0] != r[1] and r[0][-1] == r[1][-1] and any(field in pos[2] for field in ("Center", "Back")) and "Waiting" in pos[3] and self.trait(a, self.cond[2]) in tr[1]:
				...
				self.cond[2] += 1
				self.target = r[1]
		elif ("when your other  is placed from the stage to the waiting room" in t and "when your other \"" in aa) or "when your other character with  in its card name is put in your waiting room from the stage" in t:
			...
			if r[0] == r[1] and r[0] in rst and v[0] == "Reverse":  
				...
				if "level of its battle opponent is  or lower" in t or "this's battle opponent is level  or lower" in t or "level of the battle opponent of this is  or lower" in t or "level of the battle opponent is  or lower" in t:
					...
					self.ablt = 2
				elif "cost of the battle opponent of this is  or lower" in t or "this's battle opponent is cost  or lower" in t:
			...
			if dmg>0 and r[0] == r[1] and n in r[1][-1] and cnc[0] not in r[0][-1] and cnc[1]:
				if "\" is in your climax area" in aa:
			...
			if dmg >0 and r[0][-1] == r[1][-1] and n in r[0][-1] and cnc[0] not in r[0][-1] and cnc[1] and lvop[0] >= self.digit(a):
				self.cond[0] += 1
				self.ablt = 9
		elif "when damage dealt by this is not cancelled" in t:
			if dmg>0 and r[0] == r[1] and n in r[1][-1] and cnc[0] not in r[0][-1] and not cnc[1]:
				self.ablt = 9
		elif "when the damage dealt by your character is cancelled" in t:
			if dmg>0 and r[0] != r[1] and n in r[1][-1] and r[0][-1] == r[1][-1] and cnc[0] not in r[0][-1] and cnc[1]:
				self.ablt = 9
			...
			if dmg>0 and r[0] != r[1] and n in r[1][-1] and r[0][-1] != r[1][-1] and not cnc[1] and cnc[0] in r[0][-1]:
				self.dmg = dmg
				self.ablt = 9
		elif "when the damage you received is canceled" in t or "when damage taken by you is cancelled" in t:
			if dmg>0 and r[0] != r[1] and n in r[1][-1] and r[0][-1] != r[1][-1] and cnc[1] and cnc[0] in r[0][-1]:
				self.ablt = 9
		...
		if "if this is in your memory" in t or "if this is in memory" in t:
			...
			if "Center1" not in pos[1]:
				self.ablt = 0
		elif "if this is in your center stage" in t or "if this is in the center stage" in t or "if this is on your center stage" in t or "if this in on your center stage" in t or "and this is in your center stage" in t:
			if "Center" not in pos[1]:
				self.ablt = 0
	...
	def effect(self, a, t="", aa="", pl=0):
		...
		self.isnot = ""
		self.donot = ""
		if "you may pay the cost" in t or ("you may " in t and "you may not " not in t and "you may perform that action as many times as you like" not in t) or ("your opponent may" in t and "your opponent may not" not in t):
			e = ["pay", "may"]
	...
	def condition(self, a, t, aa):
		...
		if "if all your other characters have lower power than this" in t:
		...
		elif "if the character opposite this is level " in t or "if the card opposite this is level " in t or "and the character opposite this is level" in t:
			self.cond[0] += 1
			...
			c = [-1, "more", "Trait", self.trait(a, self.cond[2])]
			self.cond[2] += 2
		elif "if your opponent's center stage has  or less characters" in t:
	...
	def convert(self, a, t, aa=""):
		...
		if "rest  of your standing character" in t or "rest  of your  character" in t or "choose  of your characters and rest them" in t:
			...
			if "if you rest " in t:
				...
				self.cond[0] += 1
				if "those characters do not stand during your next stand phase" in t.split("if you rest")[0]:
		...
		if "put the top  cards of your deck in your waiting room" in t or "put the top  cards of your deck in the waiting room" in t:
			...
			if "if that card's level is the number your opponent declared" in t:
				...
				if "put that character on any position of your back stage" in t or "put that character in any slot in the back stage" in t or "put it on any position of your back stage" in t:
					d += ["extra", "do", [-16, "salvage", "ID=_x", "Stage", "Back"]]
			elif "that card is level  or higher" in t or "if it's level  or higher" in t:
			...
			if "x = the level of that card" in t:
				d.extend(["if", "extra"])
			if "you may pay the cost" in t:
			...
			self.cond[0] += 1
			if "x = the number of climax among those card" in t or "x is the number of climax cards among those card" in t or "x = # of climax cards among those card" in t or "x = # of climax cards among them" in t:
				d += ["if", "extra"]
		elif "put the top card of your opponent's deck into his or her stock" in t:
			...
			if ("put it in your hand" in t or "put it in your hand" in t) and ("put the rest in the waiting room" in t or "put the rest in your waiting room" in t):
				if "your opponent chooses  character or  event among those card" in t or "your opponent chooses  character or  event from them" in t:
					...
					if "you choose up to  card from the other cards revealed this way" in t and "put it on top of the deck" in t:
						...
						self.cond[0] += 1
				elif "your opponent chooses  character or event from among them" in t:
					d.extend(["do", [self.digit(a, self.cond[0]), "salvage", "Character_Event", "oppturn", "opp", "Revealed"]])
					self.cond[0] += 1
			...
			if "if that card is  character of the level that your opponent declared" in t:
				...
				self.cond[0] += 1
			elif "if that card is a  or  character, or " in t or "if that card is a character with  or , or " in t or "if that card is  character with  or , or " in t:
			...
			elif "put that character in any slot on your stage" in t or "put it on any position of your stage" in t:
			...
			if "shuffle that deck" in t or "shuffles his or her deck" in t:
				d = [-1, "shuffle", "opp"]
		elif "returns all other cards in their waiting room to their deck" in t or "returns all cards from their waiting room except that card to their deck" in t:
		...
		if "all cards in your hand to the deck" in t:
		...
		elif "play rock-paper-scissors with your opponent until someone wins" in t:
			c = ["janken", "winner"]
			...
			self.cond[0] += 1
		elif "you may swap a character in your waiting room with this" in t or "you may swap  character in your waiting room with this" in t:
			c = [1, "salvage", "Character", "swap", "Clock", "upto", "this"]
			if "swap  character" in t:
				self.cond[0]+=1
			...
			c = [self.digit(a, self.cond[0]), "searchopp", f"CLevel_>={self.digit(a, self.cond[0] + 1)}"]
			self.cond[0] += 2
		...
		elif self.target and ("put that character on the bottom of your opponent's deck" in t or "put that character on the bottom of the deck" in t or "put that character at the bottom of your opponent's deck" in t or "put it on the bottom of the deck" in t):
		...
		elif self.target and "that character gets + power" in t:
			...
			if "put it in your hand" in t and "put it in your stock" in t and t.count("choose  card from the revealed card")==2:
				...
				if "choose up to  character with  or , or " in t:
					...
					c.extend(["Climax", "any"])
				elif "choose up to  character from among them" in t:
			...
			if "put it face down" in t:
				c = ["mdiscard", self.digit(a, self.cond[0]), "", "opp", "flip", "down", "upto"]
				self.cond[0] += 1
			...
			if "put it in any slot on the stage" in t:
				...
				c.extend(["Stage","Back"])
			elif "put it on the stage position that this was on" in t or "put it in the slot this was in" in t:
				c.extend(["Stage", "Change"])
			...
			if " with  in the name" in t:
				...
				self.cond[2] += 2
			c = self.discard_card(c, a, t, aa)
		elif ("choose up to  character in your hand" in t or ("choose up to  character" in t and "in your hand" in t)) and ("put it on any position of your stage" in t or "put it in any slot on the stage" in t):
			...
			c = [self.digit(a, self.cond[0]), "salvage", f"Cost_<={self.digit(a, self.cond[0] + 1)}", "upto", "show"]
			self.cond[0] += 1
			...
			c = [self.digit(a, self.cond[0]), "salvage", f"Cost_<={self.digit(a, self.cond[0] + 1)}", "show", "upto"]
			self.cond[0] += 1
			if "put them in separate slots on the stage" in t:
				c.append("Stage")
			...
			if "choose  of your characters with" in t:
				gg, t = self.seperate("choose  of your characters with", a, t, aa)
			if gg:
				if "do" in c:
					c[c.index("do") + 1].extend(gg)
				else:
			...
			if "return that character to their hand" in t or "return it to the hand" in t or "return it to hand" in t or "return it to your opponent's hand" in t or "return it to their hand" in t:
			...
			if "send it to memory" in t:
				...
				if "choose  of your opponent's character" in t:
					...
					c.extend(gg)
		elif "choose  character with a level equal to or lower than your level + in your hand" in t:
			...
			if "that character gets + power and + soul" in t:
				...
				c = [self.digit(a, self.cond[0]), self.digit(a, self.cond[0] + 1), x, "power", "#", "Trait", self.trait(a, self.cond[2]), '#trait', self.trait(a, self.cond[2] + 1)]
			elif "that character gets + power" in t:
				...
				if "power, + soul, and the following ability" in t:
					c.extend(["extra", "do", [-16, self.digit(a, self.cond[0]), x, "soul", "extra", "do", [-16, self.name(a, s='a'), x, "give"]]])
					self.cond[0] += 1
		...
		elif "choose  of your other characters and this" in t or "choose  of your characters and this" in t:
			if "return them to your hand" in t:
				c = [self.digit(a, self.cond[0]), "hander"]
			...
			if "at the beginning of your next" in t:
				c.extend(["extra", "do", [-21, self.name(a, s='at'), 3, "give", "expass", self.digit(a) + 1, "ex_ID="]])
		elif "choose  of your other character" in t:
			if "put it in the waiting room" in t:
				c = [self.digit(a, self.cond[0]), "waitinger", "Other"]
			elif "return it to your hand" in t:
				c = [self.digit(a, self.cond[0]), "hander", "Other"]
			elif "that character gets the following ability" in t:
				c = [self.digit(a, self.cond[0]), self.name(a, s='a'), x, "give", "Other"]
			elif "that character gets + level" in t:
				c = [self.digit(a, self.cond[0]), self.digit(a, self.cond[0] + 1), x, "level", "Other"]
				self.cond[0] += 2
				if "level and + power" in t:
					c.extend(["extra", "do", [-16, self.digit(a, self.cond[0]), x, "power"]])
					self.cond[0] += 1
				...
				c = [self.digit(a, self.cond[0]), self.digit(a, self.cond[0] + 1), x, "X", "power", "x", self.digit(a, self.cond[0] + 1)]
				self.cond[0] += 1
				if "x =  times level of that character" in t:
					c.append("xlevel")
				...
				c = [self.digit(a, self.cond[0]), self.trait(a, self.cond[2]), x, "trait", "Other"]
				self.cond[2] += 1
			...
			if "put it in your clock" in t:
				...
				c = [self.digit(a, self.cond[0]), "stand", "swap"]
			elif "that character gets +x power" in t:
				c = [self.digit(a, self.cond[0]), self.digit(a, self.cond[0] + 1), x, "power"]
				self.cond[0] += 1
				...
				if "that character gets + soul and this gets + power" in t:
					c += ["do", [0, self.digit(a, self.cond[0] + 1), x, "power"]]
					self.cond[0] += 1
			elif "that character gets «" in aa:
			...
			if "put it on an open position of your opponent's back stage" in t or "put it in an empty slot in your opponent's back stage" in t:
				c = [self.digit(a, self.cond[0]), "salvage", f"Cost_>={self.digit(a, self.cond[0] + 1)}", "Stage", "opp", "Back", "Open", "Opp"]
				self.cond[0] += 2
			...
			if "put it in the waiting room" in t or "put it into their waiting room" in t:
				c[1] = "waitinger"
			...
			if "move it to another vacant slot" in t or "move it to another empty slot" in t or "move it to another open position of their stage" in t:
			...
			if "with level equal to or lower than your level" in t:
				c[2] = f'TraitLC_{c[2].split("_")[1]}_"<=p"_{c[2].split("_")[2]}'
			...
			if "character whose level is  or lower" in t:
				...
				self.cond[0] += 1
		elif "move this to an empty" in t or "move this to an open position" in t or "put this to an empty slot" in t or "move this to a vacant slot" in t:
			c = [1, "move", "Open", "this"]
		...
		elif ("deal the same amount of damage to your opponent" in t or "deal the same damage to your opponent" in t) and self.dmg > 0:
		...
		if "all characters in your opponent's center stage get" in t or "all your opponent's center stage characters get" in t:
			...
			elif ("x =  times # of your characters with  or " in t or "x =  times # of your characters with either  or " in t or "x =  times number of your characters with  or " in t) and "» or «" in aa:
	...
	def cont(self, a="", x=-1):
		...
		if "this gets - level while in your hand" in t or "this gets - level in your hand" in t:
			if "if your waiting room has  or less climax" in t or "if there are  or fewer climax cards in your waiting room" in t:
				...
				self.cond[1] += 2
			elif "if you have  or more  character" in t and "or more «" in aa:
			...
			self.cond[0] += 1
		elif "if you have  or more other  or  character" in t:
			...
			if (("gets + power and " in t or "get + power and" in t) and "power and \"[" in aa) or "power and the following ability" in t or "power and the following  abilities" in t:
				...
				if "power and the following  abilities" in t:
					for _ in range(self.digit(a, self.cond[0])):
						c.extend([-2, self.name(a, self.cond[5], s='a'), x, f"ability{_}"])
						self.cond[5] += 1
				else:
		...
		elif (("your opponent cannot play  from hand" in t or "your opponent cannot play  from his or her hand" in t) and "\"backup\"" in aa) or "your opponent may not play backup from hand" in t or "your opponent cannot play backup from hand" in t:
			c = [0, "", x, "no_backup", "opp"]
	...
	def limit(self, t, a):
		d = []
		if "you may have up to  cards with the same name as this in your deck" in t or "you can put up to  cards with the same card name as this in your deck" in t:
	...
	def trigger(self, a=""):
		...
		if "door" in a:
			...
			c = [-12, "[AUTO] When the next damage dealt by this card is canceled, deal one damage to your opponent", -2,"give","Climax_auto","text", "During this turn, when the next damage dealt by the attacking character that triggered this card is canceled, deal 1 damage to your opponent."]
		elif "stock" in a:
			...
			if t.startswith("[") or t.startswith(" ["):
				...
				if "the following ability for the turn. \"" in t or "the following ability. \"" in t or "get \"" in t or "gets \"" in t or "the following ability until end of turn. \"" in t or "and the following ability. \"" in t or "effects. \"" in t or "perform it. \"" in t or "power and \"" in t or "the following ability until the end of your opponent's next turn. \"" in t:
					if t.count("]") == 1 and t.index("]") < 10:
						...
						t = t.split("]")[0].split()
				elif "gets the following 2 abilities" in t:
	...
	def colour_t(self, a="", i=0, p=False):
		if not p:
			...
			t = a.lower()
		if any(f" {colour} " in t or f"non-{colour}" in t for colour in self.colour):
			for colour in self.colour:
				if f" {colour} " in t or f"non-{colour}" in t:
	...
	def discard_card(self, c, a, t, aa):
		d = []
		if ("choose  cards from your hand" in t or "choose  card in your hand" in t or "discard  card from your hand" in t or "discard  card from hand" in t or "discard  cards from your hand" in t or "choose  cards in your hand" in t or "choose  card from your hand" in t or "discard x cards from your hand" in t or "put  card from your hand" in t) and ("hand to the waiting room" in t or "put it in your waiting room" in t or "discard it to the waiting room" in t or "put them in your waiting room" in t or "from your hand to the waiting room" in t or "from your hand in your waiting room" in t):
			if "discard x cards from your hand" in t:
				d = ["do", ["discard", "x", ""]]
			else:
				d = ["do", ["discard", self.digit(a, self.cond[0]), ""]]
				self.cond[0] += 1
		elif "put  cards from your hand on top of your deck" in t:
