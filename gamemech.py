...
def list_str(lst, sep=".", sh=False):
	...
	for inx in range(len(lst)):
		ind = str(lst[inx])
		if len(lst) == 7 and (inx == 3 or inx == 4) and (lst[0] == "a" or lst[0] == "t" or lst[0] == "e"):
			temp += f"{ind}{sep}"
		elif len(ind) >= 2:
			try:
				if sh:
...
class GameMech(Widget):
	...
	def __init__(self, **kwargs):
		...
		self.skip_cpop = list(self.emptycards)
		self.cardinfo = None
		self.fav = None
		self.fav1 = None
		self.hscv = [[], [], 0]
	...
	def start_setting1(self, *args):
		...
		self.sd["debug"]["btn"] = {}
		self.sd["debug"]["room"] = Label(text="Room: ")
		...
		self.sd["sbper"] = {}
		self.sd["menu"] = {}
		self.sd["btn"]["end"] = Button(size_hint=(None, None), text="End ", on_release=self.end_current_phase, halign='center')  # line_height=0.95,
		self.sd["btn"]["end_eff"] = Button(size_hint=(None, None), text="Continue Effect", on_release=self.end_current_ability, halign='center')  # line_height=0.95,
		...
		# self.sd["btn"]["reorder_btn"] = Button(size_hint=(None, None), text="Look Next", on_release=self.look_draw, cid="r")
		...
		self.sd["btn"]["levelup_btn"] = Button(size_hint=(None, None), text="Level up", on_release=self.reflev, cid="lev")
		...
		self.sd["btn"]["label"] = Label(text="test", text_size=((self.sd["card"][0] + self.sd["padding"]) * 0.9 * starting_hand, None), font_size=self.sd["card"][1] / 5, size_hint=(1, None), markup=True)  # ,valign="middle")
		...
		self.sd["btn"]["show_info_btn"] = Button(size_hint=(None, None), text="i", on_press=self.show_info_btn, cid="info_btn")
		...
		self.sd["btn"]["ftext"] = TextInput(text="", size_hint=(4, 1))  # ,focus=True)
		...
		self.sd["other"]["copy_scv"] = ScrollView(do_scroll_x=False, size_hint=(1, 1), effect_cls="ScrollEffect")
		self.sd["other"]["copy_btn"] = Button(size_hint=(1, 0.05), text="Close", on_release=self.copy_dismiss)
		self.sd["other"]["copy_lb"] = Label(text="", valign="middle", halign="center", text_size=(Window.width * 0.9, None), size_hint=(1, None))
		...
		for card in list(self.cdsh.keys()):
			if card != "1" and card != "2":
				...
				if self.cd[card].pos_new != "" and self.cd[card].pos_new != "Hand" and self.cd[card].pos_new != "Marker":
					if "Center" in self.cd[card].pos_new or "Back" in self.cd[card].pos_new or "Waiting" in self.cd[card].pos_new or "Memory" in self.cd[card].pos_new or "Climax" in self.cd[card].pos_new:
	...
	def ref_press(self, inst, value):
		if value == "email":
			...
			webbrowser.open('https://paypal.me/pools/c/8zTRRG2IYz')
	...
	def shelve_load(self):
		if exists(f"{data_ex}/sdata.dat"):
			with shelve.open(f"{data_ex}/sdata", "c") as shdb:
				self.pd = dict(shdb["pdata"])
				self.gd = dict(shdb["gdata"])
				self.decks["1"] = list(shdb["decks1"])
				self.decks["2"] = list(shdb["decks2"])
				self.net = dict(shdb["net"])
				self.cdsh = dict(shdb["cdata"])

	def shelve_save(self):
		with shelve.open(f"{data_ex}/sdata", "n") as shdb:
			if self.gd["popup_done"][0]:
				self.gd["p_ltitle"] = str(self.sd["popup"]["popup"].title)
				self.gd["p_t"] = list(self.gd["p_l"])
			shdb["pdata"] = dict(self.pd)
			shdb["gdata"] = dict(self.gd)
			try:
				shdb["decks1"] = list(self.decks["1"])
			except KeyError:
				shdb["decks1"] = []
			try:
				shdb["decks2"] = list(self.decks["2"])
			except KeyError:
				shdb["decks2"] = []
			shdb["net"] = dict(self.net)

			ss = {}
			for cc in list(self.cd.keys()):
		...
		for player in list(self.pd.keys()):
			for i in ("b", "c"):
				for j in range(3):
					if i == "b" and j == 2:
						continue
					self.sd["act"][f"{i}{j}{player}"] = []
					self.sd["act"][f"x{i}{j}{player}"] = Button(size_hint=(None, None), cid=f"{i}{j}{player}", on_release=self.act_ability_btn, text="ACT", background_normal=f"atlas://{img_in}/other/action_bar", size=(self.sd["card"][0], self.sd["card"][1] / 4.))
					self.parent.add_widget(self.sd["act"][f"x{i}{j}{player}"])
					self.sd["act"][f"x{i}{j}{player}"].x = -Window.width
					self.sd["act"][f"x{i}{j}{player}"].y = -Window.height

	def act_ability_fill(self, player):
		for ind in self.pd[player]["Center"] + self.pd[player]["Back"]:
			if ind != "":
				self.sd["act"][f"{self.cd[ind].pos_new[0].lower()}{self.cd[ind].pos_new[-1]}{ind[-1]}"] = []
				for item in self.cd[ind].text_c:
					if item[0].startswith(act_ability) and item[1] != 0 and "[counter]" not in item[0].lower() and item[1] > -9:
						self.sd["act"][f"{self.cd[ind].pos_new[0].lower()}{self.cd[ind].pos_new[-1]}{ind[-1]}"].append(item[0])

	def act_ability_show(self, hide=False, player="", *args):
		for btn in self.sd["act"]:
			if btn.startswith("x"):
				self.sd["act"][btn].x = -Window.width
		if player:
			pp = player
		else:
			pp = self.gd["active"]
		if not hide and self.gd["phase"] == "Main" and not self.gd["noact"][pp]:
			if "ACT" not in self.gd["ability_trigger"]:
				...
				my = []
				wr = []
				stage = self.pd[pp]["Center"] + self.pd[pp]["Back"]
				for cc in stage:
					ass = False
					for item in self.cd[cc].text_c:
						if item[0].startswith(cont_ability) and item[1] != 0 and item[1] > -9:
							if "[cont] assist" in item[0].lower():
								ass = True
								break
					ss.append((self.cd[cc].status, self.cd[cc].name_t, self.cd[cc].trait_t, ass))
				for indy in self.pd[pp]["Hand"]:
					h.append((self.cd[indy].card, self.cd[indy].name_t, self.cd[indy].trait_t))
				for indmy in self.pd[pp]["Memory"]:
					my.append((self.cd[indmy].card, self.cd[indmy].name_t, self.cd[indmy].trait_t,self.cd[indmy].back,self.cd[indmy].back_info))
				for indwr in self.pd[pp]["Waiting"]:
					wr.append((self.cd[indwr].card, self.cd[indwr].name_t, self.cd[indwr].trait_t))
				for ind in stage:
					if ind != "":
						self.gd["act_pop"][ind] = []
	...
	def perform_popup_btn(self, btn, a="", net=False):
		...
		if btn is not None and a == "":
			a = btn.a
		if not net and self.gd["per_poped"][3] < 0:
	...
	def perform_popup(self, idm, *args):
		...
		self.sd["btn"]["label"].pos = (self.sd["padding"] / 2., self.sd["popup"]["p_scv"].y + self.sd["popup"]["p_scv"].size[1])  # -self.sd["padding"]*5)

		self.sd["btn"]["show_info_btn"].size = (self.sd["card"][1] / 3, self.sd["card"][1] / 3)
		self.sd["btn"]["show_info_btn"].y = ypop - self.sd["btn"]["show_info_btn"].size[1] * 2 - self.sd["padding"] * 1.5
		# self.sd["btn"]["show_info_btn"].y = self.gd["p_yscat"] - self.sd["padding"] * 2
		self.sd["btn"]["show_info_btn"].x = xscat - self.sd["card"][1] / 3 * 2 - self.sd["padding"]
		...
		if not self.gd["payed"]:
			...
			if "Discard" in self.gd["pay"] or "MDiscard" in self.gd["pay"] or "HMemory" in self.gd["pay"] or "CXDiscard" in self.gd["pay"]:
				...
				if self.gd["pay"][ind + 1] == 0:
					...
					elif disdo == "MDiscard" and "MNameC" in self.gd["pay"]:
						self.gd["search_type"] = f"NameC_{self.gd['pay'][self.gd['pay'].index('MNameC') + 1]}"
			...
			if "ClockL" in self.gd["pay"]:
				...
				self.gd["damage_trigger"] = "pay"
				self.gd["pay"].remove("ClockL")
				if card.cid[-1] != self.gd["active"]:
					self.gd["drev"] = True
			...
			if "Counter" in self.gd["ability_trigger"]:
				...
				if "plevel" in self.gd["effect"]:
					...
					if "Name=" in self.gd["effect"]:
						...
						if sum([self.cd[lv].level for lv in self.pd[ind[-1]]["Level"] if lv != ""]) < self.gd["effect"][self.gd["effect"].index("experience") + 1]:
							check = False
	...
	def pay_opponent(self, dtype=""):
		...
		if "AI_pay" in pick and "WDecker" in pick:
	...
	def check_auto_ability(self, dt=.0, rev=[""], atk="", play="", wait="", revive="", trigger="", cnc=("", False), refr="", lvup="", stacks=True, act="", batt=False, dis="", sav="", dmg=0, rst=[], brt=("", 0), lvc=""):
		# st = []
		# print("check_auto_ability",self.gd["phase"])
		# for player in list(self.pd.keys()):
		# 	for indx in self.pd[player]["Center"] + self.pd[player]["Back"]:
		# 		st.append(self.cd[indx].name
		std = ("", "", "")
		if self.gd["standby"][0]:
		...
		for r in list(rev):
			for player in list(self.pd.keys()):
				...
				if self.gd["pp"] < 0 or self.gd["phase"] in ("Attack", "Declaration"):
					cards = list(stage + self.pd[player]["Memory"])
				# if len(self.pd[player]["Clock"])>0:
				# 	cards.append(self.pd[player]["Clock"][-1])
				else:
					cards = list(stage)

				alarm = []
				if len(self.pd[player]["Clock"]) > 0:
					if "Clock" in self.cd[self.pd[player]["Clock"][-1]].icon:
						alarm.append(self.pd[player]["Clock"][-1])
				# for atext in self.cd[self.pd[player]["Clock"][-1]].text_c:
				# 	if "[auto] alarm" in atext[0].lower() or "[auto] [alarm]" in atext[0].lower() or "[auto] shift" in atext[0].lower():
				# 		alarm.append(self.pd[player]["Clock"][-1])
				# 		break
				if player == "2":
					ss = []
				...
				for ind in cards + [player] + alarm:
					...
					for item in card.text_c:
						if item[0].startswith(auto_ability) and item[1] != 0 and item[1] > -9:
							...
							if ind == "1" or ind == "2":
								z = (item[0][item[0].index("turn")+1], self.gd['turn'], "ability")
							else:
								z = (card.turn[0], self.gd['turn'], card.turn[1])
							...
							if self.gd["phase"] == "Trigger":
								ty = (self.cd[r].card, self.cd[r].trigger, self.gd["attacking"][0], self.cd[r].name_t, self.cd[r].mcolour)
								rev = [""]
							elif atk:
								r = atk
							elif play:
	...
	def stack_ability(self, *args):
		...
		if self.gd["auto_effect"] in self.gd["stack"][player]:
			...
			if self.net["game"] and player == "1" and self.gd["ability_doing"] != "confirm" and not self.gd["oppchoose"]:
				...
				self.mconnect("act")
				return False
		elif self.net["game"] and ("ACT" in self.gd["ability_trigger"] or "Event" in self.gd["ability_trigger"]) and self.gd["ability_trigger"] and player == "1" and player == self.gd["ability_trigger"].split("_")[1][-1] and not self.net["send"]:
			if "ACT" in self.gd["ability_trigger"]:
				self.net["var1"] = "act"
			elif "Event" in self.gd["ability_trigger"]:
				self.net["var1"] = "event"
		...
		if len(self.pd[player]["Res"]) > 0:
			# print("per_poper",self.gd["per_poped"],self.gd["ability_trigger"])
			if self.gd["per_poped"][-1] and "Event" in self.gd["ability_trigger"]:
				pass
			else:
				self.event_done()
		...
		# print("after", len(self.gd["stack"]["1"]), len(self.gd["stack"]["2"]))
	...
	def auto_check(self, p):
		...
		for auto in stack:
			...
			if self.gd["nomay"] and "may" in auto[1]:
				...
				if "may" in auto[1]:
					aa = 1
			elif "pay" in auto[1] and any(abl in auto[1][-1] for abl in ("decker", "reverser", "stocker", "memorier", "clocker")) and auto[1][-1][0] == -3:
				opp = auto[1][-1][auto[1][-1].index("target") + 1]
				if opp != "" and self.cd[opp].pos_new[:-1] in ("Center", "Back"):  # and self.cd[opp].pos_old != "Waiting":
					aa = 1
				else:
					aa = 0
				...
				aa = 2
				if auto[1][0] == -1:
					auto[1][0] = len([r for r in self.pd[p]["Center"] + self.pd[p]["Back"] if r != ""])
				if "lower" not in auto[1] and len(self.cont_times(auto[1], self.cont_cards(auto[1], auto[0]), self.cd)) < auto[1][0]:
					aa = 0
				elif "lower" in auto[1] and len(self.cont_times(auto[1], self.cont_cards(auto[1], auto[0]), self.cd)) > auto[1][0]:
					aa = 0
				...
				if any(name in self.cd[s].name_t for s in stage1):
					aa = 2
				else:
					aa = 0
				...
				if "HandvsOpp" in auto[1]:
					...
					if "=lower" in auto[1] and len(self.pd[auto[0][-1]]["Hand"]) > len(self.pd[op]["Hand"]):
						aa = 0
					elif "lower" in auto[1] and len(self.pd[auto[0][-1]]["Hand"]) >= len(self.pd[op]["Hand"]):
						aa = 0
					elif "lower" not in auto[1] and "=lower" not in auto[1] and len(self.pd[auto[0][-1]]["Hand"]) <= len(self.pd[op]["Hand"]):
						aa = 0
				elif "Hand" in auto[1]:
					...
					if "lower" in auto[1] and len(self.pd[pl]["Clock"]) > auto[1][0]:
						aa = 0
					elif "=" in auto[1] and len(self.pd[pl]["Clock"]) != auto[1][0]:
						aa = 0
					elif len(self.pd[pl]["Clock"]) < auto[1][0]:
						aa = 0
				elif "Memory" in auto[1]:
					if len(self.pd[pl]["Memory"]) < auto[1][0]:
						aa = 0
				elif "Waiting" in auto[1]:
					if "Climax" in auto[1] and len([s for s in self.pd[pl]["Waiting"] if "Climax" in self.cd[s].card]) < auto[1][0]:
						aa = 0
				...
				if "Name=" in auto[1]:
					...
					if sum([self.cd[lv].level for lv in self.pd[p]["Level"] if lv != ""]) >= auto[1][auto[1].index("experience") + 1]:
						aa = 2
				...
				if "Center" not in self.cd[auto[0]].pos_new:
					...
					if opp == "":
						...
						if "OP==" in auto[1] and self.cd[opp].level_t != auto[1][auto[1].index("olevel") + 1]:
							aa = 0
						elif "OPlower" in auto[1] and self.cd[opp].level_t > auto[1][auto[1].index("olevel") + 1]:
							aa = 0
						elif "OPlower" not in auto[1] and self.cd[opp].level_t < auto[1][auto[1].index("olevel") + 1]:
							aa = 0
					elif "OPtraits" in auto[1]:
						if "OPlower" in auto[1] and len([t for t in self.cd[opp].trait_t if t != ""]) > auto[1][auto[1].index("#traits") + 1]:
							aa = 0
						elif "OPlower" not in auto[1] and len([t for t in self.cd[opp].trait_t if t != ""]) < auto[1][auto[1].index("#traits") + 1]:
							aa = 0
		...
		if "pay" in lst and isinstance(lst[1], list):
			...
			if "Flip" in lst[1]:
				...
				if qty == 0:
					if st == "down" and self.cd[ind].back:
						...
						return False
					if ind not in self.pd[p]["Memory"]:
						return False
			if "Discard" in lst[1]:
			...
			if "MDiscard" in lst[1]:
				...
				if qty == 0 and ind not in self.pd[p]["Memory"]:
					...
					leff = []
					if "MNameC" in lst[1]:
						leff = ["NameC", lst[1][lst[1].index("MNameC") + 1]]
					elif "MName=" in lst[1]:
	...
	def start_game(self, *args):
		...
		self.rect.size = (Window.width + self.sd["card"][0], Window.height / 2 - self.sd["padding"] - self.sd["card"][1] / 6)  # - self.sd["menu"]["btn"].size[1])
		...
		if self.gd["debug"] and self.net["game"]:
			try:
				self.parent.add_widget(self.sd["debug"]["room"])
			except WidgetException:
				pass
			self.sd["debug"]["room"].text = f"Room: {self.net['room']}"
			self.sd["debug"]["room"].size = (self.sd["card"][0] * 3, self.sd["b_bar"].size[1])
			self.sd["debug"]["room"].center_x = Window.width / 2
			self.sd["debug"]["room"].y = Window.height - self.sd["debug"]["room"].size[1]
		...
		if self.gd["load"]:
			...
			if not self.net["game"] and not self.gd["com"]:
				self.gd["com"] = True
			self.gd["stack"] = {"1": [], "2": []}
			Clock.schedule_once(partial(self.shuffle, "0"), phase_dt)
	...
	def popup_deck_start(self, *args):
		...
		self.decks["st"]["format_spn"] = Spinner(text="-", values=("Standard", "Side", "Neo-Standard", "Title-Specific"), cid="format")
		...
		for nx in range(1, 51):
			...
			self.decks["dbtn"][f"{nx}1b+"] = Button(text="", size_hint=(None, None), opacity=0, cid=f"{nx}1+", on_release=self.add_card, pos=pos, size=size1)
			self.decks["dbtn"][f"{nx}1b-"] = Button(text="", size_hint=(None, None), opacity=0, cid=f"{nx}1-", on_release=self.remove_card, pos=pos, size=size1)
	...
	def deck_set_title(self, inst, val):
		if val != "":
			if val != "close":
				...
				if val == "all":
					# if len(se["main"]["w"]) >= 0:
					# 	Clock.schedule_once(partial(self.popup_text, "down"))
					# 	return False
					...
					for var in se["check"].keys():
						if any(var == self.decks["rv"].data[ids]["id"] for ids in range(len(self.decks["rv"].data))):
							...
							# 		temp += "~d"
							# 		temp1.append("d")
							# 	elif "-i" in item and exists(f"{cache}/{item}"):
							# 		temp += f"~i"
							# 		temp1.append("i")
					...
					# if len(se["main"]["w"]) >= 1:
					# 	if se["check"][val]["s"]!= "" and se["check"][val]["s"] not in se["main"]["w"]:
					# 		Clock.schedule_once(partial(self.popup_text, "down"))
					# 		return False
					self.net["var"] = [str(val), 0]
		...
		if "down" in t:
			...
			for s in sets:
				...
				for item in se["check"][s]:
					if "-d" in item:
						if self.multi_info["dw"]["2"].state == 'down' and exists(f"{data_ex}/{item}"):
							down = False
		...
		# self.popup_deck("ddd")
			...
			if "d" in self.decks["c"]:
				...
				if img[:-4] in other_img or ("." not in img and img in other_img):
					...
					if "." in img:
						img = img[:-4]
				elif img[:-4] in annex_img or (img in annex_img and "." not in img):
					source = "annex"
					if "." in img:
						img = img[:-4]

				if source == "main" and self.decks[self.decks["c"][1]][0].startswith("CEJ"):
					if (img in se["main"]["a"][seti] for seti in se["main"]["a"]):
						pass
					else:
						source = "other"
						img = "grey"
			...
			self.dpop[f"i{btn.cid}"].selected_c(False)
			self.decks["selected"] = ""
			self.decks["confirm"].disabled = True
			self.decks["dismantle"].disabled = True
			self.decks["create"].text = "Create new"
			...
			self.dpop[f"i{btn.cid}"].selected_c()
			...
			if "marker" in self.gd["ability_doing"]:
				...
				if "search" in self.gd["ability_doing"]:  # self.multi_info["t"]:
					...
					if "Library" in self.gd["effect"] and "top" not in self.gd["effect"] and "bottom" not in self.gd["effect"]:
						...
						if self.net["game"]:
							self.gd["shuffle_send"] = True
						self.shuffle(self.multi_info["shuffle"])
					else:
						Clock.schedule_once(self.ability_effect)
	...
	def popup_multi_info(self, field="", owner="", deck="", cards=[], t="", shuffle="", *args):
		...
		if deck != "":
			...
			if not deck[1:].startswith("CJ") and not deck[1:].startswith("CE") and sd[deck[1:]]["img"] not in other_img:
				if img[:-4] in annex_img or (img in annex_img and "." not in img):
					if "." in img:
						img = img[:-4]
		...
		for ind in self.multi_info["cards"]:
			if "sspace" in ind:
				...
				if t != "Numbers":
					...
					if not ind.endswith("0") and self.cd[ind].back and "Library" not in self.cd[ind].pos_new:
	...
	def mconnect(self, btn, dt=0):
		...
		if "connect" in var:
			...
			if ind[-1] == "1":
				...
				temp = f"och_{list_str(self.net['var'])}"  # + "_" + self.gd["ability_doing"]
				dat = {"0": f"p{self.net['player']}", "1": self.net["room"], "2": temp, "8": self.net["select"], "9": self.gd["turn"]}
		elif "oppturn" in var:
			...
			temp = f"pch_{list_str(self.net['var'])}"  # + "_" + self.gd["ability_doing"]
			dat = {"0": f"p{self.net['player']}", "1": self.net["room"], "2": temp, "8": self.net["select"], "9": self.gd["turn"]}
		...
		if "down" in var and self.net["var"][1] > 0:
			...
			for down in list(self.downloads.keys()):
				self.req[down] = UrlRequest(f"{self.downloads[down][0]}{down}", timeout=10, on_success=self.down_data, on_cancel=self.down_data_cnc, on_failure=self.failure_message, on_error=self.error_message, on_progress=self.progress_message, ca_file=cfi.where(), verify=False)
		else:
			self.net["body"] = urlencode(dat)
			self.cnet = UrlRequest(self.net["url"], req_body=self.net["body"], req_headers=headers, on_success=self.mcheck_data, timeout=10, ca_file=cfi.where(), on_failure=self.failure_message, on_error=self.error_message, verify=False)  # , debug=True, on_progress=self.progress_message) cfi.where()
		...
		if var.startswith("w"):
			...
			if var.startswith("s") and len(var) > 1:
				...
				if "lvl" in self.net["var1"]:
					...
					Clock.schedule_once(self.shuffle_start, ability_dt)
			...
			if f"z{self.net['player']}" in var[0]:
				...
				if "lvu" in var[1]:
					...
					self.level_up_done()
				elif any(var[2].startswith(s) for s in ("a", "t", "e")):
					p = []
					c = []
					if "x" not in cont[3]:
	...
	def gotodeckedit(self, *args):
		...
		if self.decks["selected"]:
			...
			if self.decks['dbuild']['img'] in other_img:
				...
				if exists(f"{cache}/{self.decks['dbuild']['img']}"):
			...
			if self.sd["popup"]["p_scv"].scroll_y < dh * 2:
				if self.sim < len(self.gd["p_fcards"]):
					...
					for xx in range(popup_max_cards):
						...
						self.cpop[temp].import_data(sc[self.gd["p_fcards"][self.sim - 1]])
	...
	def deck_building_layout(self, *args):
		...
		for item in (d_0, d_1, d_2, d_3, d_cx):
			...
			for ind in sorted(item):
				...
				self.decks["dbtn"][f"{inx}1b+"].x = xpos + self.decks["dbtn"][f"{inx}1bb"].size[0] - self.decks["dbtn"][f"{inx}1b+"].size[0]
				self.decks["dbtn"][f"{inx}1b+"].y = self.sd["b_bar"].size[1] + d_height * d_r + self.sd["padding"] / 3
	...
	def update_qty_card(self, ind):
		...
		if self.cd[ind].card == "Climax":
			...
			if self.decks["dbuild"]["deck"][inx] == 1:
				...
				self.field_btn[f"Hand{ind}"].pos = (self.cd[ind].pos[0], self.cd[ind].pos[1] - self.decks["dbuild"]["pos"][ind][2])
			self.cd[ind].x += sep
			self.cd[ind].y -= sep
		else:
			self.cd[ind].y = self.decks["dbuild"]["pos"][ind][1]
	...
	def on_touch_move(self, touch):
		if self.gd["game_start"] and (self.gd["phase"] == "Main" or self.gd["phase"] == "Climax") and self.gd["active"] == "1":
			...
			if self.gd["selected"] != "" and self.gd["selected"] in self.gd["moveable"] and self.sd["touch_down"] is not None:
				...
				if not self.gd["confirm_requirement"]:
					if card.card == "Climax" and card.mcolour not in self.pd[self.gd["active"]]["colour"] and touch.pos[1] >= self.mat[self.gd["active"]]["mat"].y:
						self.gd["touch_move_y"] = self.mat[self.gd["active"]]["mat"].y - self.sd["touch_down"][1]
					elif card.card != "Climax" and card.level != 0 and len(self.pd[self.gd["active"]]["Level"]) < card.level and touch.pos[1] >= self.mat[self.gd["active"]]["mat"].y:
						self.gd["touch_move_y"] = self.mat[self.gd["active"]]["mat"].y - self.sd["touch_down"][1]
					elif card.card != "Climax" and card.level != 0 and len(self.pd[self.gd["active"]]["Level"]) >= card.level and card.mcolour not in self.pd[self.gd["active"]]["colour"] and touch.pos[1] >= self.mat[self.gd["active"]]["mat"].y:
						self.gd["touch_move_y"] = self.mat[self.gd["active"]]["mat"].y - self.sd["touch_down"][1]
					elif card.card != "Climax" and card.level != 0 and len(self.pd[self.gd["active"]]["Level"]) >= card.level and card.mcolour in self.pd[self.gd["active"]]["colour"] and len(self.pd[self.gd["active"]]["Stock"]) < card.cost_t and touch.pos[1] >= self.mat[self.gd["active"]]["mat"].y:
						self.gd["touch_move_y"] = self.mat[self.gd["active"]]["mat"].y - self.sd["touch_down"][1]
					else:
						...
						if width[0] < card.center_x < width[1] and height[0] < card.center_y < height[1]:
							if field != card.pos_new:
								if self.pd[self.gd["active"]][field[:-1]][int(field[-1])] != "":
									if self.pd[self.gd["active"]][field[:-1]][int(field[-1])] in self.gd["moveable"]:
	...
	def field_btn_fill(self, dt=0):
		...
		for player in list(self.pd.keys()):
			...
			for field in self.gd["stage"]:
				...
				pos = (-Window.width * 3, self.mat[player]["field"][field][1] - self.sd["padding"] + self.mat[player]["mat"].y)
	...
	# 	self.field_btn[f"{field}{player}"].source = f"atlas://{img_in}/other/movable"
	# for r in range(select2cards):
	# 	self.field_btn[f"stage{r}{player}s"].source=f"atlas://{img_in}/other/select2"
	...
	def debug_btn(self, dt=0):
		# debug button
		debug_btn_size = Window.width / 10  # self.sd["card"][0] * 0.8
		...
		for item in self.gd["d_btn_list"] + self.gd["d_btn_2"]:
			if len(item) == 1:
				...
				cid = f"{item.split('_')[0][:2]}{item.split('_')[1][:1]}"
				if len(item) > 5:
					short = cid
				else:
					short = item
				self.sd["debug"]["btn"][f"d{item}"] = Button(text=short, on_release=self.debug_effect, cid=cid)
			if item in self.gd["d_btn_2"]:
				y1 = 2
			else:
				y1 = 0
			...
			if "Opp" in item:
				if len(item) == 1:
					self.sd["debug"]["btn"][f"d{item}"].x = self.sd["debug"]["btn"][f"d{item}"].size[0] * (self.gd["inx"] - 1)
				else:
					self.sd["debug"]["btn"][f"d{item}"].x = self.sd["debug"]["btn"][f"d{item}"].size[0] * (self.gd["inx"] - 1) - debug_btn_size / 3
				self.sd["debug"]["btn"][f"d{item}"].y = self.sd["debug"]["btn"][f"d{item}"].size[1] + self.sd["debug"]["btn"][f"d{item}"].size[1] * y1
				if self.gd["inx"] >= 10:
					self.gd["inx"] = 1
			else:
				if len(item) == 1:
					...
					self.sd["debug"]["btn"][f"d{item}"].x = self.sd["debug"]["btn"][f"d{item}"].size[0] * self.gd["inx"] - debug_btn_size / 3
				self.sd["debug"]["btn"][f"d{item}"].y = 0 + self.sd["debug"]["btn"][f"d{item}"].size[1] * y1
				self.gd["inx"] += 1
	...
	def debug_effect(self, btn):
		...
		if self.sd["debug"]["btn"][f"d{self.gd['d_btn_list'][0]}"].text == "":
			return
		if self.gd["active"] == "1":
			self.gd["moveable"] = []
		player = self.gd["active"]
		...
		if "Da" in btn.cid:
			...
			if len(inds) > 0:
				...
				if card.status != "Reverse":
					card.reverse()
					self.check_auto_ability(rev=[card.ind], rst=[card.ind])
		elif "RS" in btn.cid:
			inds = [s for s in self.pd[player]["Center"] + self.pd[player]["Back"] if s != "" and self.cd[s].status != "Stand"]
			if len(inds) > 0:
				ind = choice(inds)
				card = self.cd[ind]
		...
		for field in self.gd["fields"]:
			for ind in self.pd[player][field]:
				...
				for item in self.cd[ind].text_c:
					if item[0].startswith(cont_ability) and item[1] != 0 and item[1] > -9:
						eff = ab.cont(item[0])
						if "no_move" in eff and self.cd[ind].pos_new != "Hand":
							aa = False
							break
				if aa:
	...
	def update_playable_climax(self, player):
		...
		for ind in self.pd[self.gd["active"]]["Hand"]:
			...
			# add colour req
			if self.cd[ind].card == "Climax":
				if self.gd["anyClrclimax"][self.gd["active"]] or self.cd[ind].mcolour in self.pd[self.gd["active"]]["colour"]:
	...
	def end_current_phase(self, *args):
		...
		if self.gd["astock_select"]:
			...
			Clock.schedule_once(self.pay_mstock)
		elif "Effect" in self.sd["btn"]["end"].text or "Reveal" in self.sd["btn"]["end"].text or "Heal" in self.sd["btn"]["end"].text or "Stock" in self.sd["btn"]["end"].text:
			if self.gd["ability_doing"] in self.gd["ability_effect"]:
			...
			if len(player) > 1:
				...
				if self.gd["auto_effect"] in self.gd["stack"][player]:
					...
					if self.net["game"] and player == "1" and self.gd["ability_doing"] != "confirm" and not self.gd["oppchoose"]:
						...
						Clock.schedule_once(self.shuffle_start)
				else:
					Clock.schedule_once(self.shuffle_start)
		else:
			Clock.schedule_once(self.shuffle_start)
	...
	def clocker(self, *args):
		...
		# if self.net["game"]:
		# 	if self.gd["target"][0][-1] == "2" and ind[-1] == "2":
		# 		self.gd["target"][0] = f"{self.gd['target'][0][:-1]}1"
		# 	elif self.gd["target"][0][-1] == "1" and ind[-1] == "1":
		# 		self.gd["target"][0] = f"{self.gd['target'][0][:-1]}2"
	...
	def wind(self, *args):
		...
		if self.net["game"] and "Opp" in self.gd["effect"]:
	...
	def send_to(self, field, ind, pos=None):
		if "Waiting" in field:
			self.send_to_waiting(ind)
		else:
			self.check_pos(ind)
	...
	def damage(self, *args):
		...
		if self.gd["damage"] > 0 or self.gd["damage_refresh"] > 0:
			...
			if self.gd["damageref"]:
				if not self.gd["both"] and self.gd["damage_refresh"] >= 0 and len(self.pd[player]["Library"]) <= 0 and len(self.pd[player]["Clock"]) >= 7:
				...
				if self.gd["cancel_dmg"]:
					...
					if "oncancelput" in self.gd["effect"]:
						if "ocLevel" in self.gd["effect"]:
							if "oclower" not in self.gd["effect"] and self.cd[temp].level>=self.gd["effect"][self.gd["effect"].index("ocLevel")+1]:
								cc = True
							elif "oclower" in self.gd["effect"] and self.cd[temp].level<=self.gd["effect"][self.gd["effect"].index("ocLevel")+1]:
								cc = True
					if cc:
						self.send_to("Clock",temp)
					else:
	...
	def effect_to_stage(self, s, dt=0):
		ind = self.gd["ability_trigger"].split("_")[1]
		if (ind[-1] == "1" and "oppturn" not in self.gd["effect"]) or (ind[-1] == "2" and "oppturn" in self.gd["effect"]):
			...
			self.gd["status"] = self.add_to_status(f"{s}1", self.gd["effect"])
			self.select_field()
	...
	def search(self, dt=0):
		...
		if self.gd["p_c"] != "" and (not self.gd["target"] or self.gd["p_again"]):
			...
			if "NameSet" in self.gd["search_type"]:
			...
			elif "TraitLCost" in self.gd["search_type"]:
				...
				if "TextN" in self.gd["search_type"]:
					name = self.gd['search_type'].split('_')[-1]
					search += f" or \"{name}\""
			elif "EachNameC" in self.gd["search_type"]:
				name = self.gd['search_type'].split('_')[1:]
				for x in range(len(name)):
					if x == 0:
						search += f"character with \"{name[x]}\" in name"
					else:
						search += f" and {uptomay}character with \"{name[x]}\" in name"
			elif "Name" in self.gd["search_type"]:
				if len(self.gd["search_type"].split("_")) == 3:
					search = f"\"{self.gd['search_type'].split('_')[1]}\" or \"{self.gd['search_type'].split('_')[2]}\" card"
				else:
					search = f"\"{self.gd['search_type'].split('_')[1]}\" card"
			else:
				if len(self.gd['search_type'].split('_')) > 1:
					for rr in range(len(self.gd['search_type'].split('_'))):
						if rr == 0:
							search += f"{rr}"
						else:
							search += f" or {rr}"
			...
			for r in range(self.gd["effect"][0]):
				...
				if "Stage" in self.gd["effect"]:
					if st:
						...
						for rr in range(select2cards):
							self.field_btn[f"stage{rr}{ind[-1]}s"].pos = (-Window.width * 2, -Window.height * 2)
	...
	def salvage(self, dt=0):
		...
		if self.gd["p_c"] != "" and (not self.gd["target"] or self.gd["p_again"]):
			...
			if "Stage" in self.gd["effect"]:
				...
				if len(self.gd["target_temp"]) > 0:
					...
					if "Resonance" in self.gd["effect"]:
						for rr in self.gd["resonance"][1]:
							self.gd["target"].append(rr)
					elif len(self.pd[imd[-1]][self.gd["effect"][self.gd["effect"].index(swap)]]) > 0:
			...
			if "_standby" in self.gd["search_type"]:
				...
				if "TraitN=" in self.gd["search_type"]:
				...
				if "_<=p" in self.gd["search_type"]:
					search = f"level {len(self.pd[ind[-1]]['Level'])} or lower character"
			...
			# 	search = f"{self.gd['search_type'].split('_')[1]} or {self.gd['search_type'].split('_')[2]}"
			elif "Name=_" in self.gd["search_type"] or "Name_" in self.gd["search_type"]:
				...
				if len(self.gd['search_type'].split('_')) == 2:
					search = f"{self.gd['search_type'].split('_')[0]} {self.gd['search_type'].split('_')[1]}"
				else:
					search = self.gd["search_type"]
			...
			if "if" in self.gd["effect"]:
				if lif or ("swap" in self.gd["effect"] and not lif):
					...
					if len(lif) < self.gd["effect"][self.gd["effect"].index("if") + 1]:
						self.gd["done"] = False
					if "ifLevel" in self.gd["effect"] and all(self.cd[cc].level < self.gd["effect"][self.gd["effect"].index("ifLevel") + 1] for cc in lif):
						self.gd["done"] = False
			elif "do" in self.gd["effect"] and self.gd["do"][0] > 0:
				self.gd["done"] = True
	...
	def select_filter_check(self, ind):
		if ind in self.gd["target"]:
			return False
		if "Colour" in self.gd["search_type"]:
			if "ColourCx" in self.gd["search_type"] and "Climax" in self.cd[ind].card and self.cd[ind].mcolour in self.gd["search_type"]:
				return True
			elif self.cd[ind].mcolour in self.gd["search_type"]:
				return True
			...
			if "Character" in self.cd[ind].card and "<=p" in self.gd["search_type"] and self.cd[ind].level <= len(self.pd[ind[-1]]["Level"]) and self.cd[ind].cost <= int(self.gd["search_type"][-1]) and any(tr in self.cd[ind].trait_t for tr in self.gd["search_type"].split("_")[1:-1]):
				return True
		elif "TraitL" in self.gd["search_type"]:
			if "_<=p" in self.gd["search_type"] and any(tr in self.cd[ind].trait_t for tr in self.gd["search_type"].split("_")[1:-1]) and self.cd[ind].level <= len(self.pd[ind[-1]]["Level"]):
				return True
			elif "_<=" in self.gd["search_type"] and "p" not in self.gd["search_type"][-1] and self.gd["search_type"] and any(tr in self.cd[ind].trait_t for tr in self.gd["search_type"].split("_")[1:-1]) and self.cd[ind].level <= int(self.gd["search_type"][-1]):
				return True
			elif "_>=" in self.gd["search_type"] and "p" not in self.gd["search_type"][-1] and self.gd["search_type"] and any(tr in self.cd[ind].trait_t for tr in self.gd["search_type"].split("_")[1:-1]) and self.cd[ind].level >= int(self.gd["search_type"][-1]):
				return True
			elif "_=" in self.gd["search_type"] and "p" not in self.gd["search_type"][-1] and self.gd["search_type"] and any(tr in self.cd[ind].trait_t for tr in self.gd["search_type"].split("_")[1:-1]) and self.cd[ind].level == int(self.gd["search_type"][-1]):
			...
			if self.gd["search_type"].split("_")[1] in self.cd[ind].trait_t or self.gd["search_type"].split("_")[2] in self.cd[ind].trait_t or self.gd["search_type"].split("_")[-1] in self.cd[ind].name_t:
				return True
		elif "TraitN" in self.gd["search_type"]:
			if self.gd["search_type"].split("_")[1] in self.cd[ind].trait_t or self.gd["search_type"].split("_")[-1] in self.cd[ind].name_t:
				return True
		elif "BTrait" in self.gd["search_type"]:
			...
			if self.gd["search_type"].split("_")[1:] == [""] and len(self.cd[ind].trait_t) <= 0 and "Character" in self.cd[ind].card:
				return True
			...
			if any(name in self.cd[ind].name_t for name in self.gd["search_type"].split("_")[1:]):
				return True
		elif "Name" in self.gd["search_type"] and any(name in self.cd[ind].name_t for name in self.gd["search_type"].split("_")[1:]):
			return True
			...
			if any(any(txt.lower() in text.lower() for txt in self.gd["search_type"].split("_")[1:-1]) for text in self.cd[ind].text_o) or self.gd["search_type"].split("_")[-1] in self.cd[ind].name_t:
				return True
		elif "Text" in self.gd["search_type"] and any(any(txt.lower() in text.lower() for txt in self.gd["search_type"].split("_")[1:]) for text in self.cd[ind].text_o):
			return True
		elif "Card" in self.gd["search_type"] and self.cd[ind].mcolour.lower() in self.gd["search_type"]:
			return True
		elif "NCost" in self.gd["search_type"]:
			if "Character" in self.cd[ind].card and "<=" in self.gd["search_type"] and self.cd[ind].cost <= int(self.gd["search_type"][-1]) and self.gd["search_type"].split("_")[1] in self.cd[ind].name_t:
				return True
		elif "Cost_<=" in self.gd["search_type"] and self.cd[ind].cost <= int(self.gd["search_type"][-1]) and "Character" in self.cd[ind].card:
			return True
		elif "Cost_>=" in self.gd["search_type"] and self.cd[ind].cost>= int(self.gd["search_type"][-1]) and "Character" in self.cd[ind].card:
			return True
			...
			if "_standby" in self.gd["search_type"] and self.cd[ind].level_t <= len(self.pd[ind[-1]]["Level"]) + 1 and "Character" in self.cd[ind].card:
				return True
			elif "_<=p" in self.gd["search_type"] and "Character" in self.cd[ind].card and self.cd[ind].level_t <= len(self.pd[ind[-1]]["Level"]):
				return True
			...
			if "_<=p" in self.gd["search_type"] and self.cd[ind].level_t <= len(self.pd[ind[-1]]["Level"]):
				...
				return True
			elif "_>=" in self.gd["search_type"] and self.cd[ind].level_t >= int(self.gd["search_type"][-1]):
				return True
	...
	def info_start(self, *args):
		...
		if self.infob:
			self.infob.cancel()
			self.infob = None
		if self.gd["phase"] in ("", "Janken") and self.gd["popup_done"][1]:  # @End , "Draw", "Standby", "Stand Up", "Mulligan"
			pass
				...
				if self.gd["btn_id"][:-1] in self.gd["stage"]:
					...
					if cc:
						if self.check_back_hidden(self.cd[cc]):
							self.cardinfo.import_data(self.cd[cc], annex_img)
	...
	def popup_text(self, c="", dt=0):
		...
		if c in ("Attack", "Declaration"):
			...
			ptext = "Waiting for to create room..."
		elif "Loading" in c:
			ptext = "Loading..."
		elif "mstock" in c:
		...
		if "waiting" not in c and "making" not in c and "Loading" not in c:
			...
			if "LoadGame" in c or "no_internet" in c or "no_error" in c:
				self.sd["text"]["close"].size = (self.sd["card"][0] * 2, self.sd["card"][1] / 2.)
				self.sd["text"]["retry"].size = (self.sd["card"][0] * 2, self.sd["card"][1] / 2.)
				self.sd["text"]["close"].center_x = xscat / 4
				self.sd["text"]["retry"].center_x = xscat / 4 * 3
				self.sd["text"]["retry"].y = self.sd["padding"] * 1.5
			else:
				self.sd["text"]["close"].center_x = xscat / 2.
		elif c == "waiting" and self.net["game"] and all(self.gd["phase"] != phase for phase in ("Janken", "Mulligan", "")):
			self.sd["text"]["close"].y = self.sd["padding"] * 1.5
			self.sd["text"]["label"].y = self.sd["padding"] * 3.5 + self.sd["card"][1] / 2.
			self.sd["text"]["close"].center_x = xscat / 2.
	...
	def popup_pl(self, phase):
		...
		if self.gd["p_f"] and any(s in self.gd["p_c"] for s in pl):
			...
			if "Character" in self.gd["search_type"] or "Climax" in self.gd["search_type"] or "Event" in self.gd["search_type"]:
				self.gd["p_l"] = [s for s in p_l if self.cd[s].card in self.gd["search_type"]]
			elif "ColourCx" in self.gd["search_type"]:
				self.gd["p_l"] = [s for s in p_l if "Climax" in self.cd[s].card and self.cd[s].mcolour in self.gd["search_type"]]
			elif "Colour" in self.gd["search_type"]:
				self.gd["p_l"] = [s for s in p_l if self.cd[s].mcolour in self.gd["search_type"]]
			elif "ID=" in self.gd["search_type"]:
				...
				if "TraitLCost" in self.gd["search_type"]:
					level = self.gd["search_type"].split("_")[-2]
					cost = self.gd["search_type"].split("_")[-1]
				trait = self.gd["search_type"].split("_")[1:-1]
				if "<=p" in level:
					if "TraitLCost" in self.gd["search_type"]:
						self.gd["p_l"] = [s for s in p_l if "Character" in self.cd[s].card and self.cd[s].level_t <= len(self.pd[self.gd["p_owner"]]["Level"]) and self.cd[s].cost_t <= int(cost) and any(tr in self.cd[s].trait_t for tr in trait)]
					else:
				...
				if name in trait:
					trait.remove(name)
				self.gd["p_l"] = [s for s in p_l if trait in self.cd[s].trait_t or name in self.cd[s].name_t]
			elif "Trait" in self.gd["search_type"]:
				if self.gd["search_type"].split("_")[1:] == [""]:
					self.gd["p_l"] = [s for s in p_l if len(self.cd[s].trait_t) <= 0 and "Character" in self.cd[s].card]
				else:
					self.gd["p_l"] = [s for s in p_l if any(trait in self.cd[s].trait_t for trait in self.gd["search_type"].split("_")[1:])]
			...
			elif "NameC" in self.gd["search_type"]:
				...
				self.gd["p_l"] = [s for s in p_l if any(any(txt.lower() in text.lower() for txt in self.gd["search_type"].split("_")[1:-1]) for text in self.cd[s].text_o) or self.gd["search_type"].split("_")[-1] in self.cd[s].name]
			elif "Text" in self.gd["search_type"]:
				...
				level = self.gd["search_type"].split("_")[-1]
				if "<=" in level:
					self.gd["p_l"] = [s for s in p_l if ("Character" in self.cd[s].card and self.cd[s].level <= int(level[-1])) or "Event" in self.cd[s].card]
			elif "CLevelN" in self.gd["search_type"]:
				name = self.gd["search_type"].split("_")[-1]
				level = self.gd["search_type"].split("_")[1]
				if "_standby" in level:
					self.gd["p_l"] = [s for s in p_l if "Character" in self.cd[s].card and name in self.cd[s].name_t and self.cd[s].level <= len(self.pd[self.gd["p_owner"]]["Level"]) + 1]
				elif "<=" in level:
					self.gd["p_l"] = [s for s in p_l if "Character" in self.cd[s].card and name in self.cd[s].name_t and self.cd[s].level <= int(level[-1])]
			elif "CLevel" in self.gd["search_type"]:
				if "_standby" in self.gd["search_type"]:
					self.gd["p_l"] = [s for s in p_l if "Character" in self.cd[s].card and self.cd[s].level_t <= len(self.pd[self.gd["p_owner"]]["Level"]) + 1]
				elif "<=" in self.gd["search_type"]:
					if "p" in self.gd["search_type"][-1]:
						self.gd["p_l"] = [s for s in p_l if self.cd[s].level_t <= len(self.pd[s[-1]]["Level"]) and "Character" in self.cd[s].card]
					else:
						self.gd["p_l"] = [s for s in p_l if self.cd[s].level_t <= int(self.gd["search_type"][-1]) and "Character" in self.cd[s].card]
				elif ">=" in self.gd["search_type"]:
					self.gd["p_l"] = [s for s in p_l if self.cd[s].level_t >= int(self.gd["search_type"][-1]) and "Character" in self.cd[s].card]
			elif "Level" in self.gd["search_type"]:
				if "_standby" in self.gd["search_type"]:
				...
				if "<=" in self.gd["search_type"]:
					self.gd["p_l"] = [s for s in p_l if self.cd[s].cost_t <= int(self.gd["search_type"][-1]) and "Character" in self.cd[s].card]
				elif ">=" in self.gd["search_type"]:
					self.gd["p_l"] = [s for s in p_l if self.cd[s].cost_t >= int(self.gd["search_type"][-1]) and "Character" in self.cd[s].card]
			elif "CTrigger" in self.gd["search_type"]:
			...
			self.gd["p_f"] = True
			self.sd["btn"]["show_all_btn"].text = "Filter"  # self.gd["search_type"].split("_")[0].upper().replace("=","")
			if "Markers" in self.gd["p_c"]:
				if self.gd["p_c"].split("_")[-1] in self.pd[self.gd["p_owner"]]["marker"]:
					self.gd["p_l"] = [s[0] for s in self.pd[self.gd["p_owner"]]["marker"][self.gd["p_c"].split("_")[-1]]]
				else:
	...
	def popup_filter(self, *args):
		...
		if starting_hand < len(self.gd["p_l"]) <= 7:
			...
			if len(self.sd["popup"]["popup"].title) > (6 * starting_hand):
				tt += round(len(self.sd["popup"]["popup"].title) / 6 / len(self.gd["p_l"]))
			...
			if len(self.sd["popup"]["popup"].title) > (6 * starting_hand):
				tt += round(len(self.sd["popup"]["popup"].title) / 6 / popup_max_cards)
			if len(self.gd["p_l"]) > 7 and len(self.sd["popup"]["popup"].title) > (6 * starting_hand) and tt > 3:
				tt -= 1
		else:
			self.gd["p_hand"] = int(starting_hand)
			if len(self.sd["popup"]["popup"].title) > (6 * starting_hand):
				tt += round(len(self.sd["popup"]["popup"].title) / 6 / starting_hand)
		# print("tt")
		...
		elif (any(s in self.gd["p_c"] for s in ("Search", "Salvage", "Encore")) or ("Discard" in self.gd["p_c"] and self.gd["search_type"])) and "swap" not in self.gd["effect"]:
			self.gd["p_title"] += self.sd["card"][1] / 3 + self.sd["padding"] * 2

		self.gd["p_yscat"] = self.gd["p_yscv"] + self.gd["p_title"] + self.sd["card"][1] / 3 + self.sd["padding"] * 3  # + self.gd["p_height"]/
		...
		if self.gd["notarget"] and not self.gd["p_f"]:
			...
			pos = (self.sd["padding"] * 1, self.sd["padding"] * 6.5 + self.sd["card"][1] / 2)
		...
		# self.gd["p_yscat"] += self.sd["padding"] * 1.5 + self.sd["card"][1] / 2. + self.sd["btn"]["label"].size[1]
			...
			if "opp" in self.gd["effect"]:
				opp = "opponets's "
			if "hand" in self.gd["p_c"] or "clock" in self.gd["p_c"]:
				if "fix" in self.gd["p_c"]:
					fix = ""
				else:
					fix = "up to "
				...
				if "show" in self.gd["effect"]:
					...
					if "stacked" in self.gd["effect"]:
						...
						self.sd["btn"]["label"].text = f"Look at {fix}{self.gd['p_look']} cards from top of your deck. Choose {upto}{self.gd['p_max_s']} of them and put it in your hand, and put the rest in the waiting room."
			elif "waiting" in self.gd["p_c"] or "bdeck" in self.gd["p_c"] or "tdeck" in self.gd["p_c"] or "tbdeck" in self.gd["p_c"] or "climax" in self.gd["p_c"]:
		...
		for inx in range(len(self.gd["p_l"])):
			...
			if "sspace" in ind:
				...
				if "Numbers" not in self.gd["p_c"]:
					if "Add" in self.gd["p_c"]:
						...
						if "Stage" in self.gd["effect"] and ind in self.gd["target"]:
							...
							for r in range(select2cards):
								if self.field_btn[f"stage{r}{ind[-1]}s"].x < 0 and self.field_btn[f"stage{r}{ind[-1]}s"].y < 0:  # (-Window.width * 2, -Window.height * 2):
				...
				if "Add" not in self.gd["p_c"] and not ind.startswith("n") and not ind.startswith("t") and self.cpop[ind].level != self.cd[ind].level_t:
		...
		if "Image" not in self.gd["p_c"]:
			...
			if self.gd["ability_trigger"]:
				self.sd["btn"]["show_info_btn"].size = (self.sd["card"][1] / 3, self.sd["card"][1] / 3)
				self.sd["btn"]["show_info_btn"].y = self.gd["p_ypop"] - self.sd["btn"]["show_info_btn"].size[1] * 2 - self.sd["padding"] * 1.5
				# self.sd["btn"]["show_info_btn"].y = self.gd["p_yscat"] - self.sd["padding"] * 2
				self.sd["btn"]["show_info_btn"].x = self.gd["p_xscat"] - self.sd["card"][1] / 3 * 2 - self.sd["padding"]
			if (any(item in self.gd["p_c"] for item in ("Search", "Salvage", "Encore")) or ("Discard" in self.gd["p_c"] and self.gd["search_type"])):# and "swap" not in self.gd["effect"]:
		...
		if "Image" not in self.gd["p_c"]:
			...
			if any(item in self.gd["p_c"] for item in ("Clock", "Level", "Hand", "Search", "Salvage", "Counter", "Encore", "Discard", "Marker")):
				...
				if "top" in self.gd["effect"] and ("bottom" in self.gd["effect"] or "Waiting" in self.gd["effect"]):  # and "bottom" in self.gd["effect"]
	...
	def update_text_selected(self):
		...
		for i in range(len(self.gd["chosen"])):
			if "Look" in self.gd["p_c"] and "reorder" in self.gd["p_c"]:
				if i == 0:
		...
		if self.multi_info["t"]:
			...
			if not self.multi_info["t"]:
				...
				if "auto" in self.gd["p_c"]:
					...
					if (any(s in self.gd["p_c"] for s in ("Salvage", "Search", "Encore", "Discard")) or ("Look" in self.gd["p_c"] and ("hand" in self.gd["effect"] or "clock" in self.gd["effect"]))) and btn.cid not in self.gd["chosen"]:
					...
					if self.gd["p_c"] == "Hand" or self.gd["p_c"] == "Levelup":
						...
						if "Mulligan" in self.gd["p_c"]:
							...
							if "hand" in self.gd["p_c"]:
								if "stacked" in self.gd["p_c"]:
									self.sd["btn"]["Look_btn"].text = "Take selected stack"
								else:
								...
								self.sd["btn"]["Look_btn"].text = f"Return {word}"
							else:
							...
							if self.gd["resonance"][0]:
								...
								self.sd["btn"][f"{phase}_btn"].text = "Select card"
		...
		if self.sd["cpop_slc"] != "":
			...
			if self.decks["dbuilding"]:
				...
				if self.check_back_hidden(self.cd[self.sd["cpop_slc"]]):
					self.cardinfo.import_data(self.cd[self.sd["cpop_slc"]], annex_img)
		else:
			if self.sd["cpop_pressing"] is not None:
				self.sd["cpop_pressing"].cancel()
				self.sd["cpop_pressing"] = None
		...
		if self.gd["notarget"]:
			...
			if len(max) < self.gd["p_max_s"]:
				if "hand" in self.gd["effect"] and "upto" in self.gd["effect"]:
					self.sd["btn"][f"{phase}_btn"].disabled = False
				elif len(self.gd["chosen"]) < len(max):
					self.sd["btn"][f"{phase}_btn"].disabled = True
				else:
					self.sd["btn"][f"{phase}_btn"].disabled = False
			else:
	...
	def check_event(self, ind):
		...
		for item in ablts:
			...
			if "play" in play:
				if play[0] == -1:
					# print(play[play.index("Name")+1],self.cd[ind].name_t)
	...
	def check_condition(self, ind):
		...
		text = ""
		if card.card == "Climax":
			if self.gd["anyClrclimax"][self.gd["active"]] or card.mcolour in self.pd[ind[-1]]["colour"]:
				check = True
			else:
				text = "Colour"
		else:
			if card.level_t == 0:
				...
				if card.mcolour in self.pd[ind[-1]]["colour"] or (self.gd["anyClrChname"][self.gd["active"]] and card.card == "Character" and any(nn in card.name for nn in self.gd["anyClrChname"][self.gd["active"]])):
					if len(self.pd[ind[-1]]["Stock"]) >= card.cost_t:
						check = True
					else:
						text = "Cost"
		...
		if card.card == "Event" and (self.gd["noevent"][ind[-1]] or not self.check_event(ind)):
			effect = False
			text = "Effect"
		elif card.card == "Character" and not self.check_event(ind):
			effect = False
			text = "Effect"
		elif card.card == "Climax" and self.gd["noclimax"][ind[-1]]:
			effect = False
			text = "Effect"
		...
		if check and effect:
			...
			if self.gd["confirm_requirement"]:
				...
				# self.sd["btn"]["label"].text_size = (self.gd["p_xscat"] * 0.9, None)
				if card.card == "Event" and self.gd["noevent"][ind[-1]]:
					self.sd["btn"][
	...
	def stack_btn_perform(self, qty):
		for inx in range(len(self.sd["sbper"]), qty + 1):
			self.sd["sbper"][f"{inx}"] = Labelbtn(size=(self.sd["card"][0] * 5.5 + self.sd["padding"], self.sd["card"][1] * 2.25))
	...
	def confirm_popup(self, dt=.0, ind="", c="", icon="", a="", o=""):
		...
		if "encore" in self.gd["p_c"]:
			...
			self.sd["btn"]["effect_btn"].center_x = xscat / 4. * 3 - self.sd["padding"] * 0.75  # - self.sd["card"][0] / 2 + self.sd["padding"]
	...
	def janken_done(self, *args):
		if self.janken_done_check:
			return
		if self.gd["j_result"] != 0 and self.gd["turn"] == 0:
			self.janken_done_check = True
	...
	def look_top(self, btn):
		...
		if btn is not None and btn == "s":
			...
			# elif "reorder" in self.gd["effect"] and "waiting" not in self.gd["effect"] and "bdeck" not in self.gd["effect"]:
			# 	print(999)
			# 	qty = self.gd["effect"][0]
			# 	c += "_reorder"
				...
				if "stacked" in self.gd["effect"]:
					...
					# 	self.gd["oppchoose"] = False
					c += "_stacked"
			elif "tbdeck" in self.gd["effect"]:
				qty = self.gd["effect"][self.gd["effect"].index("tbdeck") + 1]
				c += "tbdeck"
			elif "tdeck" in self.gd["effect"]:
				qty = self.gd["effect"][self.gd["effect"].index("tdeck") + 1]
				c += "tdeck"
			...
			if self.net["game"] and "looktopopp" in self.gd["effect"] and not self.net["send"]:
				...
				if btn is not None and btn == "l":
					...
					if len(chosen) < len(self.gd["p_l"]):
						for r in range(len(self.gd["p_l"]) - len(chosen)):
							if "reorder" in self.gd["effect"] and ("waiting" in self.gd["effect"] or "bdeck" in self.gd["effect"]) and "waity" not in self.gd["effect"]:
								chosen.append("R")
				...
				if ("reorder" in self.gd["effect"] and ("waiting" in self.gd["effect"] or "bdeck" in self.gd["effect"]) and "reorder" not in self.gd["p_c"]) or ("tbdeck" in self.gd["effect"] and "bdeck" in self.gd["effect"] and "tbdeck" not in self.gd["p_c"]):
					if len(self.gd["p_l"]) > len(self.gd["chosen"]):
						...
						if "tbdeck" in self.gd["effet"]:
							self.gd["effet"].remove("tbdeck")
							self.gd["effet"].append("bdeck")
						if "tbdeck" in self.gd["p_c"]:
							self.gd["p_c"] = self.gd["p_c"].replace("tbdeck","bdeck")
					...
					if ind == "W":
						...
						if "stacked" in self.gd["effect"] and "hand" in self.gd["effect"]:
							...
							if "hand" in self.gd["effect"]:
								...
								self.pd[ind[-1]]["Library"].remove(ind)
								self.pd[ind[-1]]["Library"].insert(-1, ind)
								self.stack(ind[-1])
							elif "bdeck" in self.gd["effect"]:
								self.pd[ind[-1]]["Library"].remove(ind)
								self.pd[ind[-1]]["Library"].insert(0, ind)
	...
	def look_top_done(self, dt=0):
		...
		if "if" in self.gd["effect"]:
			self.gd["done"] = True
			if "ifLevel" in self.gd["effect"] and all(self.cd[cc].level<self.gd["effect"][self.gd["effect"].index("ifLevel") + 1] for cc in self.gd["if"]):
				self.gd["done"] = False
	...
	def play_card(self, *args):
		...
		if self.net["game"] and not self.net["send"] and self.gd["active"] == "1":
			...
			card.turn = [int(self.gd["turn"]), card.pos_old]

			if len(self.pd[self.gd["active"]]["Stock"]) >= card.cost_t and card.card != "Climax":
				self.pay_stock(card.cost_t)
				self.gd["payed"] = True
				self.gd["stock_payed"] = True
				to_remove_c = []
				for cost in card.cost_c:
					if cost[1] < 0:
						if self.cont_remove(cost, self.gd["play_card"], self.gd["play_card"][-1]) and cost not in to_remove_c:
			...
			if card.card == "Character":
				...
				if "cards" in self.gd["effect"]:
					aa = False
					if "Hand" in self.gd["effect"]:
						if "lower" not in self.gd["effect"] and len(self.pd[card.ind[-1]]["Hand"]) >= self.gd["effect"][0]:
							aa = True
						elif "lower" in self.gd["effect"] and len(self.pd[card.ind[-1]]["Hand"]) <= self.gd["effect"][0]:
							aa = True
					if aa:
						self.gd["effect"] = self.gd["effect"][self.gd["effect"].index("do") + 1]
					else:
	...
	def ability_event(self, dt=0, m=""):
		...
		if "pay" in self.gd["effect"] and not m:
			...
			self.gd["mtrigger"] += self.gd["effect"][1]
		elif "drawupto" in self.gd["effect"]:
			self.gd["draw_upto"] = self.gd["effect"][self.gd["effect"].index("drawupto") + 1]
			if self.gd["draw_upto"] == "x":
				if "xsodlevel+" in self.gd["effect"]:
					self.gd["effect"][1] = int(self.gd["numbers"]) + self.gd["effect"][self.gd["effect"].index("xsodlevel+") + 1]
		...
		elif "search" in self.gd["effect"] or "searchopp" in self.gd["effect"] or "stsearch" in self.gd["effect"]:
	...
	def ability_effect(self, *args):
		# print("_"*50)
		# print(self.gd["do"])
		# print("self.gd['effect']",self.gd["done"],self.gd["effect"])
		...
		if self.net["game"] and self.gd["ability_trigger"] and self.gd["ability_trigger"].split("_")[1][-1] == "2" and "oppturn" not in self.gd["effect"]:
			self.gd["choose"] = True
			self.gd["p_owner"] = "2"
		...
		if self.gd["done"] and self.gd["do"][0] > 0:
			...
			self.gd["effect"] = list(self.gd["effect"][self.gd["effect"].index("dont") + 1])
			self.gd["done"] = False
			self.gd["payed"] = True
			...
			# if self.net["game"] and self.gd["confirm1"][0] and ind[-1] == "1" and not self.net["send"] and not self.gd["confirm2"][0]:
			# 	self.net["var"] = ["a", str(self.gd["auto_effect"][0]),
			# 	                   self.gd["stack"][player].index(self.gd["auto_effect"]), [],
			# 	                   [], 1]
			#
			# elif self.net["game"] and self.gd["confirm1"][0] and ind[-1] == "1" and self.net["send"] and not self.gd["confirm2"][0]:
			# 	if self.gd["show_wait_popup"]:
			# 		Clock.schedule_once(partial(self.popup_text, "waiting"))
			# 	self.mconnect("counter")
			# elif self.net["game"] and self.gd["confirm1"][0] and ind[-1] == "2" and not self.net["send"] and not self.gd["confirm2"][0]:
			...
			if self.net["game"] and "oppturn" in self.gd["effect"]:
				if ind[-1] == "1":
					...
					if not self.gd["target"]:
						self.gd["p_c"] = ""
					self.cardnum()
			elif self.gd["com"] and (ind[-1] == "2" or "oppturn" in self.gd["effect"]):
				self.gd["p_c"] = "Numbers"
				pick = self.ai.ability(self.pd, self.cd, self.gd)
			...
			ind = self.gd["ability_trigger"].split("_")[1]
			if "xclock/2" in self.gd["effect"]:
				self.gd["effect"][1] = int(len(self.pd[ind[-1]]["Clock"]) / 2)
				self.gd["effect"].remove("xclock/2")
			...
			if self.gd["effect"][0] == -1:
				self.gd["effect"][0] = len([r for r in self.pd[idm]["Center"] + self.pd[idm]["Back"] if r != ""])

			mm = True
			if "lower" not in self.gd["effect"] and len(self.cont_times(self.gd["effect"], self.cont_cards(self.gd["effect"], idm), self.cd)) < self.gd["effect"][0]:
				mm = False
			elif "lower" in self.gd["effect"] and len(self.cont_times(self.gd["effect"], self.cont_cards(self.gd["effect"], idm), self.cd)) > self.gd["effect"][0]:
				mm = False
			...
			if "if" in self.gd["effect"]:
				if "iftotal" in self.gd["effect"] and self.gd["ifcount"]>=self.gd["effect"][self.gd["effect"].index("if")+1]:
					self.gd["done"] = True
				elif wait:
					self.gd["done"] = True
			elif "do" in self.gd["effect"] and self.gd["do"][0] > 0:
			...
			# elif self.gd["com"] and (ind[-1] == "2" or ("oppturn" in self.gd["effect"] and ind[-1] == "1")):
			# 	self.gd["p_c"] = "Discard"
				...
				if "Waiting" in self.gd["effect"]:
					...
					if "xClimax" in self.gd["effect"]:
						...
						if len(self.gd["extra"]) > 0:
							tx = self.cd[self.gd["extra"][0]].level + self.gd["effect"][self.gd["effect"].index("xLevel+x") + 1]
					self.gd["extra"] = []
			...
			if self.gd["effect"][0] == "x" or (isinstance(self.gd["effect"][0], int) and self.gd["effect"][0] > 0 and self.gd["effect"][2][-2:] == "=x"):
				if "xrlevel+1" in self.gd["effect"]:
					...
					self.gd["effect"][2] = f"{self.gd['effect'][2][:-1]}{self.cd[self.gd['reveal_ind']].level}"
					...
					self.gd["extra"] = []
				elif "xrqlevelc" in self.gd["effect"] and self.gd["effect"][0] == "x":
					lv = 0
					for rr in self.gd["extra"]:
						if "Character" in self.cd[rr].card and self.cd[rr].level == self.gd["effect"][self.gd["effect"].index("xrqlevelc") + 1]:
							lv += 1
					self.gd["effect"][0] = lv
					self.gd["salvage"] = self.gd["effect"][0]
					self.gd["extra"] = []
					...
					if self.gd["extra"] and "swap" in self.gd["effect"]:
						lv = self.cd[self.gd['extra'][0]].level - self.gd["effect"][self.gd["effect"].index("xslevel-") + 1]
						if lv < 0:
							lv = 0
						self.gd["effect"][2] = f"{self.gd['effect'][2][:-1]}{lv}"
						self.gd["move"] = self.cd[self.gd["extra"][0]].pos_old
				...
				if "xResonance" in self.gd["effect"]:
					if "xcolourdiff" in self.gd["effect"]:
						self.gd["effect"][2] = "_".join(["ColourCx"] + [cc not in list(set([self.cd[s].mcolour for s in self.gd["resonance"][1] if s != ""])) for cc in self.colour])
					else:
						self.gd["effect"][2] = "_".join(["ColourCx"] + list(set([self.cd[s].mcolour for s in self.gd["resonance"][1] if s != ""])))
				self.gd["search_type"] = str(self.gd["effect"][2])
			# elif "ID=_x" in self.gd["effect"] and self.gd["effect"][0] == -16:
			# 	print(91)
			# 	self.gd["effect"][0] = len(self.gd["extra"])
			# 	self.gd["salvage"] = int(self.gd["effect"][0])
			...
			self.update_movable(self.gd["active"])
		elif "done" in self.gd["ability_effect"]:
	...
	def rest(self, dt=0):
		...
		if self.gd["effect"][0] == 0:
			if ind[-1] == "1" or (ind[-1] == "2" and self.gd["com"]):
				if self.cd[ind].status != "Rest" and ("Center" in self.cd[ind].pos_new or "Back" in self.cd[ind].pos_new):
					self.gd["target"].append(ind)
				else:
					self.gd["target"].append("")
			self.gd["effect"][0] = 1
				...
				for item in self.cd[sm].text_c:
					if item[0].startswith(cont_ability) and item[1] != 0 and item[1] > -9:
						...
						if "no_move" in eff and self.cd[sm].pos_new != "Hand":
							...
							break
				if aa:
					self.gd["moveable"].append(sm)
			if all(ss in self.gd["moveable"] for ss in s):
				...
				self.cd[s[1]].setPos(field=self.mat[s[1][-1]]["field"][self.cd[s[0]].pos_old], t=self.cd[s[0]].pos_old)
				self.pd[s[1][-1]][self.cd[s[0]].pos_old[:-1]][int(self.cd[s[0]].pos_old[-1])] = s[1]
			self.gd["moveable"] = []
			...
			if "Battle" in power:
				...
				if opp != "":
					...
					if "OPtraits" in power:
						if "OPlower" not in power and len([t for t in self.cd[opp].trait_t if t != ""]) < power[power.index("#traits") + 1]:
							pp = True
						elif "OPlower" in power and len([t for t in self.cd[opp].trait_t if t != ""]) > power[power.index("#traits") + 1]:
							pp = True
					...
					if "Experience" in power:
						if "eName=" in power and len(self.cont_times(power, self.pd[opp[-1]]["Level"], self.cd)) < power[power.index("Experience") + 1]:
							pp = True
						elif sum([self.cd[lv].level for lv in self.pd[opp[-1]]["Level"] if lv != ""]) < power[power.index("Experience") + 1]:
							pp = True
				...
				if "opp" in power and ind[-1] == "1":
					...
					opp = ind[-1]
				if len(self.pd[opp]["Clock"]) < int(power[power.index("Clocks") + 1]):
					pp = True
	...
	def cont_add(self, peff, ind, player, instage=True):
		...
		if peff[0] == 0:
			...
			if opp != "":
				...
				if "OPtraits" in peff:
					if "OPlower" not in peff and len([t for t in self.cd[opp].trait_t if t != ""]) < peff[peff.index("#traits") + 1]:
						return
					elif "OPlower" in peff and len([t for t in self.cd[opp].trait_t if t != ""]) > peff[peff.index("#traits") + 1]:
						return
	...
	def check_no_effect(self, effect, ind):
		...
		for rr in ("act", "clock", "Clrclimax","ClrChname", "encore", "event", "backup"):
			if f"no_{rr}" in effect or ("Clr" in rr and f"any_{rr}" in effect):
				...
				if rr in ("backup", "event"):
					if "Battle" in effect and self.gd["phase"] in self.battle:
						self.gd[f"no{rr}"][e] = True
					elif "Battle" not in effect:
						self.gd[f"no{rr}"][e] = True
				else:
					if "any_Clrclimax" in effect:
						self.gd[f"any{rr}"][e] = True
					if "any_ClrChname" in effect:
						self.gd[f"any{rr}"][e].append(effect[effect.index("Name")+1])
					else:
						self.gd[f"no{rr}"][e] = True
	...
	def check_cont_ability(self, dt=0, act=True, *args):
		...
		self.gd["anyClrclimax"] = {"1": False, "2": False}
		self.gd["anyClrChname"] = {"1": [], "2": []}
		self.gd["markerstock"] = []
		self.gd["contadd"] = {}
			...
			if card.back:
				...
				if "not" in self.gd["effect"]:
					...
					if "Colour" in self.gd["effect"]:
						if self.gd["effect"][self.gd["effect"].index("Colour")+1] in card.mcolour:
							rr = True
				...
				if "if" in self.gd["effect"]:
					...
					if "x#" in self.gd["effect"]:
						...
						if self.gd["effect"] and ncx >= cx:
							self.gd["done"] = True
					elif "sameoppnum" in self.gd["effect"]:
						if self.cd[self.gd["mill_check"][0]].level == int(self.gd["numbers"]):
							self.gd["done"] = True
					elif "extra" in self.gd["effect"]:
						for temp in self.gd["mill_check"]:
							self.gd["extra"].append(temp)
			...
			if self.gd["draw_upto"] <= 0:
				...
				if "AUTO" in self.gd["ability_trigger"] or "ACT" in self.gd["ability_trigger"] or "Event" in self.gd["ability_trigger"]:
					...
					if all(self.pd[p]["done"]["Mulligan"] for p in list(self.pd.keys())):
						...
						Clock.schedule_once(self.stand_phase, move_dt_btw)
					else:
					...
					if ef and "astock" in ef:
						...
						if "Name" in ef:
							if ind[-1] not in self.gd["astock"][ef[ef.index("Name")+1]]:
								self.gd["astock"][ef[ef.index("Name") + 1]] = {"1": [], "2": []}
	...
	def update_marker(self, p=""):
		...
		for player in p:
			...
			for ind in self.pd[player]["marker"]:
				...
				for ind1 in self.pd[player]["Clock"] + self.pd[player]["Center"] + self.pd[player]["Back"]:
					card1 = self.cd[ind1]
					if ind1 == "" or ind1 in self.pd[player]["marker"]:
						continue
					self.mat[player]["mat"].remove_widget(card1)
					self.mat[player]["mat"].add_widget(card1)

	def remove_marker(self, ind="", q=0, s=None, m=""):
		idm = ""
		...
		if ind in self.pd[ind[-1]]["marker"]:
			...
			for item in rev:
				...
				for m in sx:
					...
					if idm and self.cd[idm].name in self.gd[f"{m}tock"] and (ind, item[0]) in self.gd[f"{m}tock"][self.cd[idm].name][ind[-1]]:
						self.gd[f"{m}tock"][self.cd[idm].name][ind[-1]].remove((ind, item[0]))
	...
	def marker(self, *args):
		...
		if "Return" in self.gd["effect"]:
			if card in self.pd[card[-1]]["marker"] and len(self.pd[card[-1]]["marker"][card]) >= self.gd["effect"][0]:
				if self.gd["effect"][0] == -1 and "Waiting" in self.gd["effect"]:
			...
			if opp_ind != "":
				...
				for text in card.text_c:
					...
					if "no_reverse" in effect:
						if "cost" in effect and card_opp.cost_t <= effect[effect.index("cost") + 1]:
							reverse[0] = False
				...
				for text in card_opp.text_c:
					effect = ab.cont(text[0])
					if "no_reverse" in effect:
						if "cost" in effect and card.cost_t <= effect[effect.index("cost") + 1]:
							reverse[1] = False
						elif "event" in effect:
							reverse[1] = False
						elif "olevel" in effect:
					...
					if "no_attack" in effect:
						if "Clock" in effect:
							if "lower" in effect and len(self.pd[ind[-1]]["Clock"]) > effect[effect.index("Clock") + 1]:
								aa = False
							elif "lower" not in effect and len(self.pd[ind[-1]]["Clock"]) < effect[effect.index("Clock") + 1]:
								aa = False
	...
	def counter_step_main(self, *args):
		...
		if not self.gd["nobackup"][self.gd["opp"]] or not self.gd["noevent"][self.gd["opp"]]:
			for c in cards:
				if c != "":
		...
		if self.gd["active"] == "2":
			for s in self.pd[self.gd["opp"]]["Hand"]:
				...
				if card.icon == "Counter":
					if card.card == "Character" and not self.gd["nobackup"][self.gd["opp"]] and self.gd["counter_icon"][self.gd["opp"]][0]:
						for text in card.text_c:
					...
					elif card.card == "Event" and "[counter]" in card.text_c[0][0].lower() and not self.gd["noevent"][self.gd["opp"]] and self.gd["counter_icon"][opp][1] and len(self.pd[opp]["Level"]) >= card.level and card.mcolour in self.pd[opp]["colour"] and len(self.pd[opp]["Stock"]) >= card.cost_t:
	...
	def attack_phase(self, *args):
		...
		if "Attack" in self.gd["skip"]:
			...
			if self.gd["turn"] == 1:
				self.gd["d_atk"][0] = 1
				if self.gd["debug"] and self.gd["d_3atk_1turn"] and not self.net["game"]:
					self.gd["d_atk"][0] = 3
			self.gd["attack"] = len([s for s in self.pd[self.gd["active"]]["Center"] if self.cd[s].status == "Stand"])
	...
	def climax_phase_play(self, dt=0):
		...
		if self.net["game"] and self.gd["active"] == "2" and len(self.pd[self.gd["active"]]["Climax"]) <= 0:
			...
			self.sd["btn"]["end"].y = 0
			self.update_playable_climax(self.gd["active"])
			if len(self.gd["playable_climax"]) < 1 or self.gd["noclimax"][self.gd["active"]]:
				if self.net["game"] and self.gd["active"] == "1":
					self.net["var"] = ["x"]
	...
	def level_up(self, *args):
		...
		if self.gd["clocker_rev"]:
			...
			if self.gd["rev"] or self.gd["drev"]:
				player = self.gd["opp"]
			else:
				player = self.gd["active"]
	...
	def level_up_done(self, *args):
		self.sd["popup"]["popup"].dismiss()
		if self.net["game"] and self.net["lvlsend"] and self.gd["chosen"][0][-1] == "1":  # and self.gd["active"] == "1":
			...
			if self.gd["chosen"]:
				...
				self.send_to("Level",card,pos="up")
	...
	def clock_phase(self, *args):
		...
		if self.net["game"] and self.gd["active"] == "2":
			...
			if discard == "pass" or self.gd["noclock"][self.gd["active"]]:
				self.end_current_phase()
			else:
			...
			if self.gd["noclock"][self.gd["active"]]:
		...
		if self.gd["both"]:
			...
			for p in w:
				...
				if len(self.pd[p]["Library"])<=0 and len(self.pd[p]["Waiting"])<=0 and len(self.pd[p]["Res"])<=0:
					w[p] = True
		elif (len(self.pd[player]["Clock"]) >= 7 and len(self.pd[player]["Level"]) >= 3) or len(self.pd[player]["Level"]) >= 4 or (len(self.pd[player]["Library"])<=0 and len(self.pd[player]["Waiting"])<=0 and len(self.pd[player]["Res"])<=0):
			if player == "1":
				self.gd["wl"] = False
			else:
				self.gd["wl"] = True
	...
	def discard(self, dt=0):
		...
		if self.gd["p_c"] != "" and not self.gd["target"]:
			...
			if "Climax" in self.gd["search_type"] or "Character" in self.gd["search_type"]:
				disc = f"{self.gd['search_type'].lower()} {word}"
			elif "Cost" in self.gd["search_type"]:
				...
				trait = self.gd['search_type'].split('_')[1:-1]
				if "_<=p" in self.gd["search_type"]:
					disc = f"level {len(self.pd[imd[-1]]['Level'])} or lower"
				elif "_<=" in self.gd["search_type"]:
					disc = f"level {self.gd['search_type'][-1]} or lower"
				elif "_>=" in self.gd["search_type"]:
					disc = f"level {self.gd['search_type'][-1]} or higher"
				elif "_=" in self.gd["search_type"]:
					disc = f"level {self.gd['search_type'][-1]}"
				for rr in range(len(trait)):
					if rr == 0:
						disc += f" with «{trait[rr]}»"
					else:
						disc += f" or «{trait[rr]}»"
				disc += " character"
			elif "Trait" in self.gd["search_type"]:
				if len(self.gd["search_type"].split("_")) == 3:
					disc = f"«{self.gd['search_type'].split('_')[1]}» or «{self.gd['search_type'].split('_')[2]}» character"
				elif self.gd["search_type"].split("_")[1:] == [""]:
					disc = f"character with no traits"
				else:
					disc = f"«{self.gd['search_type'].split('_')[1]}» character"
			elif "Name=" in self.gd["search_type"]:
				if len(self.gd['search_type'].split('_')) == 3 and self.gd["discard"] == 1:
					disc = f"\"{self.gd['search_type'].split('_')[1]}\" or \"{self.gd['search_type'].split('_')[2]}\" {word}"
				else:
					disc = f"\"{self.gd['search_type'].split('_')[1]}\" {word}"
			elif "Name" in self.gd["search_type"]:
				disc = f"charcter with \"{self.gd['search_type'].split('_')[1]}\" in its card name"
			elif "CLevel" in self.gd["search_type"]:
				if "_standby" in self.gd["search_type"]:
					disc = f"level {len(self.pd[imd[-1]]['Level']) + 1} or lower"
				elif "=p" in self.gd["search_type"]:
					disc = "level equal to your level or "
				else:
					if "CLevelN" in self.gd["search_type"]:
						disc = f"level {self.gd['search_type'].split('_')[1][-1]} or "
					else:
				...
				disc += " character"
				if "CLevelN" in self.gd["search_type"]:
					disc += f" with \"{self.gd['search_type'].split('_')[-1]}\" in its card name"
			else:
				disc += f"{word}"
			...
			if self.gd["resonance"][0] and len(self.gd["resonance"][1]) < self.gd["resonance"][2]:
				...
				if "Name=" in self.gd["effect"] or "Name=" in self.gd["pay"]:
					self.sd["popup"]["popup"].title = f"Discard {self.gd['discard']} {disc}"
		...
		for card in cards:
			...
			if move:
				if many:
					...
					xpos = self.mat[owner]["mat"].size[0] / 2. - len(cards) / 2. * width + self.sd["padding"] / 2. + width * self.gd["inx"]
				elif len(cards) % 2 != 0:
					xpos = self.mat[owner]["mat"].size[0] / 2. - (len(cards) - 1) / 2. * width - self.sd["card"][0] / 2. + width * self.gd["inx"]
